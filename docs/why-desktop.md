...
- 兼容、扩展窗口和增强三种呈现模式。兼容模式在独立 Desktop frame 下保留上游默认客户端；扩展窗口使用 Desktop 自有 layout/sidebar surface 承载官方 occupant 并形成倒 L；增强模式使用同一套 Desktop 自有布局与内部 caption。Desktop frame 还会按能力提供原生材质与拖动区域。
