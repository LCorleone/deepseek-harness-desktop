...
不会。仓库固定一个未修改的官方 Harness 上游版本。兼容模式在独立 overlay frame 下运行上游默认 Web client；扩展窗口与增强模式通过插件/profile composition 边界安装 Desktop 自有 root layout，并继续承载官方 slot occupant。所有模式都不会直接修改上游源码。
