...
- **清单迁移（2026-09-15，issue #010）**：接缝级重放点清单已迁至活文档 `dev-log/upstream-upgrade-replay-checklist.md`（补齐 P11/P14/P15/P16/P6/P7/P9/dshmarket 八类 + Step 0「diff 上游 defaults/接触面」程序）——本段估算为工作量框架，接缝明细以活文档为准、随接缝新增持续更新。
...
**首单上线（2026-09-04 晚，P7 全线闭环）**：free-search 0.4.182 经 tarball 通道真机安装成功、搜索可用（bing→ddg）。发布履历：sequence 11（0.4.181，真机暴露四连坑后作废）→ 12（0.4.182 修复版，Windows/Linux 摘要对拍一致 adce37b4…）。**真机排障四坑（全修复入库）**：①CLI 锁定 add 闸门误伤市场通道自己的 file: 目标（修=受信交接 env DSH_COMPANY_TARBALL_HANDOFF+CLI 双验放行，用户手敲仍拒）②Windows 杀软瞬时锁 settings.yaml 致 EPERM fatal（修=dsh-atomic-write yarn 补丁退避重试，普惠消除一类随机崩溃）③market service 裸 catch 吞断言原因（修=内联 cause detail）④包内 dsh.bundle.patch 无 './' 前缀与清单条目严格相等失配（修=0.4.182 对齐生态形态+管线级「包声明↔条目一致」断言防再犯）。另：digest workflow 学会携带 tarball 产物（跨平台字节取证）；旧包缓存曾致摘要假分歧（清缓存后一致）。**流程注记**：装机失败的排障依赖日志增强三轮迭代——安装失败细节现已完整落 error.log 与 UI。

**0.4.183 上线验收（2026-09-05 午，sequence 13 真机一次装成+anysearch 实测可用；沙箱联网同批实证）——P7 市场线全部闭环。**
**0.4.183 三键制定案（2026-09-05 用户拍板）**：链=tavily→exa→anysearch 纯键制（用户自注册免费 key：tavily 1,000 次/月、exa $20+$10/月、anysearch 1,000 次/天）；bing/ddg 引擎移除（质量差/公司封禁，抓取源全否）。key 政策定案：key 不入客户端分发（客户端持有必可提取）。**沙箱 shell 网络修复**：企业强制代理+TLS 检查环境下沙箱 curl/node 两关全挂——主进程自注入（代理 env+NODE_USE_ENV_PROXY+CA bundle+NO_PROXY），架构 note .agents/notes/implemented/architecture/2026-09-05-corporate-network-env 双语。
**搜索源运营现状（2026-09-05，已被 0.4.183 取代免费链部分）**：免费链 bing（质量差）→ddg（公司封禁）实际不可用；tavily/exa 走**用户自注册免费额度**（key 不入客户端分发——客户端持有必可提取；Tavily 1,000 credits/月、Exa $10/月免费，设置节自配，链天然支持混配）；免费兜底候选=so360 引擎（实测无验证码质量好，0.4.183 待拍板，需限速+so.com/link 跳转处理）。环境变量 TAVILY_API_KEY/EXA_API_KEY 回退是设计行为（注意与用户机器既有 env 的交互——排障时先查 env）。
...
- **机制主干=profile 换新（2026-09-09 13:48 用户定案，优于卸载手术）**：旧 profile 整目录改名靠边（profiles/desktop.bak-<时间戳>，留取证/回滚）→ 走现成首次运行机制（materializer+pnpm install，fleet 验证成熟路径）全新重建→boot。零 pnpm remove 手术（今天实证其脆弱：缺 tarball 即 ENOENT）；用户数据无忧（会话/settings 在 ~/.dsh 根不随 profile 走）；2026-09-09 真机已实证（用户删 .dsh→应用重建→host 干净启动）。
- 附带清理：市场回执/state（home settings.yaml 装权台账+序列棘轮）需同步清或验证市场对「回执在而 profile 空」的呈现（倾向清，回到全部可装态）；自动层记 telemetry（plugin_reset/profile_recreate）。
- 不动：~/.dsh 根的 settings/sessions/SSO/声明 ack；公司市场本体。
...
**已核实事实**：①目录条目键=packageName@version，manifest **本身可表达多版本**（allowlist 列表 + 按名/版本排序）；②市场 `companyPackageKey = 包名`（单版本视图，刻意设计）→ 双版本会被折叠；③boot 精确查找（名+版本）**双版本各找各的**✓，但「有更新」分类 `.find(名)` 取版本序最小者=**单钉假设**。
...
**状态（2026-09-06 晚，全链真机闭环）**：beta v1（seq 14）部署+applied 日志实证；**首个 beta-only 预发 free-search 0.4.184（seq 15）全链走通**——含 #59 安装闸门雷修复（子进程 handoff 验签 stable∪beta）与 P10 更新提示（通知/横幅/直装/单重启五环真机过）。tag v0.3.2-beta-channel。**P10（更新提示）同日落地**：boot 拒绝分类+upgrade-in-place 直装替换+市场更新横幅。待：0.4.184 浸泡→promote（群预告纪律）。
