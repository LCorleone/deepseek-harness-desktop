...
**搜索源运营现状（2026-09-05）**：免费链 bing（质量差）→ddg（公司封禁）实际不可用；tavily/exa 走**用户自注册免费额度**（key 不入客户端分发——客户端持有必可提取；Tavily 1,000 credits/月、Exa $10/月免费，设置节自配，链天然支持混配）；免费兜底候选=so360 引擎（实测无验证码质量好，0.4.183 待拍板，需限速+so.com/link 跳转处理）。环境变量 TAVILY_API_KEY/EXA_API_KEY 回退是设计行为（注意与用户机器既有 env 的交互——排障时先查 env）。
