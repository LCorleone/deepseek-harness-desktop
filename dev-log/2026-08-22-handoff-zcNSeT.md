...
**机制来源**：参考 cowork-nova-tauri（dowork 移植）实测代码。公司 SSO 门户 sdp.deloitte.com.cn，app_id=1005+内置 app_key（复用，已拍板）。两路径：A 静默（whoami /upn → SignEntity POST /web/dai/token，注意本质是 app_key 握手非真 SSO，只做便捷加速）；B 浏览器手动（loopback 回调 + code_challenge + 回调签名校验 verify=sha256(token+ts+username+app_key+email)，±10min）= 真 SSO，门禁以此为准。
