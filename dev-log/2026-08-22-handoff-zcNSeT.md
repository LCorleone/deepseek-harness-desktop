...
**已确认（2026-09-09 21:21 用户）**：桌面图标名显示 `Deloitte DSH Desktop` ✓（b80 实测）。
...
- **Phase 2 市场按 runtime 选版+安装闸** `deda379cdd` → 评审 APPROVED（留 1×P2+2×P3）→ 修批与本文档并行落地（overlay 不兼容整包消失钉测试/representability 次序/market 侧谓词去重，market 文件与 Phase 3 不相交，收官报告补号→`59c5148b2f`）。
...
### 2026-09-10 下午战报（15:25）
**P11 全卡关闭 + b84**：A/B/C 三段全落地过审（B 评审零发现）；政策开关砍除（构建期 policy 非远端，digest fail-closed 已自动禁面）。b84=161.8MB（+5.8MB pip/virtualenv）。遥测实证 `python_runtime {available:true, version:3.12.10}`（b85/b86 各一条）。
**WAL 连环安装阻断破案（b83 起）**：b81 第二装报 "another plugin install recovery transaction is pending" ——安装恢复 WAL 的 'awaiting-restart' 相位要等下次启动验证，而 `beginLocked` 对任何现存事务一律拒 ⇒ 同会话连装 by-design 不可能。修 `a15399bf0b`（按相位分派：sealed 事务被新安装链式取代，新 preimage=当前=上次成功后的状态，失败回滚到它；真待恢复相位照拒）+ 修批 `a942c9dc6a`（先采集后原子取代，消除零 WAL 窗口）。**b83 真机实证**：09:46:29 装 free-search → 09:47:34 装 sidebar（同会话 65 秒、无重启）✓；终端 `python --version`=3.12.10 ✓。
**P16 沙箱升权弹窗 + 共享 Python 环境（b85→b86）**：设计经 scout（上游已有 danger-full-access 升权模式与 denied 分类器、宿主在 Electron 主进程可直接弹窗）。实现 `f8a5930d1b` → 评审 2×P1（provisioning 写 `__pycache__` 毁 digest；本地基底 `-m virtualenv` 必败）+ 修 `0755d44c50`（`PYTHONDONTWRITEBYTECODE`；两级基底=本地 `venv --copies` → 捆绑 `virtualenv`；pyenv 自愈）→ 复审 APPROVED。共享环境=`%LOCALAPPDATA%\DSH Desktop\pyenv`（宿主静默建；pip 装包进它，捆绑树零写入=digest 完好）。
**b85 真机首败→b86 修好**：b85 上 agent 跑 pip → 弹窗未出现（无父窗）→ 120s 超时 → agent 误判网络改后台重试 → 永卡。破案：pip 在沙箱内**挂死**（不报错）⇒「事后弹窗」永不触发。双修：①`0cbe74c5b7` 弹窗挂主窗+托盘唤出、中止即取消不记去重、preset 弹窗预告；②`28b798883e` **dsh-pip 事前闸**（沙箱内秒退并打 `Access is denied` 喂给弹窗机制，绝不调真 pip）；评审 APPROVED+2×P2 → `89957cbe37`（macOS 隐藏态 `app.show()`、gate 入口分包安全 `DSH_PIP_GATE_ENTRY`、判据盲区注释）。**b86 用户验收 OK**。
**P6「公司 skill 当插件发」解冻（轻防护版）**：2026-09-02 原方案（三层重防护）砍到只留「明文不落盘」。scout 六缝全绿（原生 provider/inject(['skills'])/opaque resourceBase/插件形态/`ctx.tools.register`+stdin 管道不经沙箱/model-gateway blob 三钥先例/分发链零改动 + `type:'skill'` 枚举已存在）。brief=`dev-log/briefs/2026-09-10-p6-company-skills.md`（四批）。**批① 落地**（`b41655f16f`+修批 `ae0aa90bea`，39 测试）：容器格式 `{version:1, skills:[N 个单-skill 文档]}`、packer/unpacker、密钥策略（沿用固定混淆 key+env，README 明示「共享 key→可互解」缺口）、release-surface 守卫（剪枝盲区已修：`pkg/lib/unpack.mjs` 现在能抓）。**批② 容器插件 `dsh-company-skills` 在跑**（provider 列 N 条、按需只解一个、inject 注册）。批③ 脚本通道、批④ 首批 skill 发布待做。
**待确认**：`sandbox_escalation` 遥测在库中【无记录】——b86 的 pip 弹窗链是否实际走通？（若走通却无事件=遥测未接线，需查；若只是装了包没跑 pip 链，则以 agent 侧记录补验）

### 2026-09-10 晚间战报（17:10）
**Settings preset 噪音回归（0.1.2 同类雷）**：真机 b86 报告「Agent presets 里冒出几个点不动的 preset」。根因=上游 `packages/preset/agent-presets/src/index.ts:104-112` 在 0.1.2 线新增 `includeShippedRoot`（zod 默认 **true**），我们锁定构建的补丁只写 `roots:[公司目录]`（0.1.1 线上足够隐藏上游 preset）⇒ shipped 根仍被扫，standard/ptc/cordis/minimal 冒出来且在锁定组合里 compose 不了=不可选。修：GUI 侧 `profile.ts` 增 `includeShippedRoot:false`（`59de0ca6c3`）→ 评审抓出 **CLI 锁 overlay 同病**（`cli-lock/desktop-cli-lock.patch.yml` 另造 preset 行）→ CLI 侧同修 + face marker（`8c59bc2bd0`）。评审另核「0.1.2 其他默认值反转」（inspector/plugin-inventory/subagent-sdk）不在我们补丁接触面。**与备份 profile 无关**。
**P6 skill 当插件发（轻防护）三批落地**：批① 容器格式+打包/校验+release-surface 守卫（`b41655f16f` + 评审 P1 修 `ae0aa90bea`：剪枝盲区 `pkg/lib/unpack.mjs` 现在能抓；`main` 纳入发布面）；批② 容器插件 `dsh-company-skills`（`1b336b2703`，评审 APPROVED；`inject(['skills'])` 注册 provider / list 列 N 条 / get 按需解密 / opaque resourceBase；明文 fixtures/tests/scripts/src 一律不发且有断言）；批③ 脚本执行通道 `company_skill_run`（`08be0af5c5` → 评审 P1：解释器只查 PATH 在打包机必挂 → 修 `b5fe4ae1ef`：桌面侧注入 `DSH_DESKTOP_NODE_EXECUTABLE`/`DSH_DESKTOP_PYTHON_EXECUTABLE` 绝对路径 + 插件侧 env→PATH(win32 只认 .exe/.com)→execPath(`ELECTRON_RUN_AS_NODE`) 顺序；→ 复审批 APPROVED 后我补 `9c5d0c363e`：注入值加存在性探测（陈旧 env 不遮蔽回退）+ win32 `.cmd` 跳过用例）。**批④（打包首批 skill + 发布）阻塞在「要内置哪些 skill」的输入**；**company-skills 未进 allowlist/plugin-sources ⇒ 市场暂不可见（b88 无此面）**。
**P16 系统提示重构**：`ff86917a4d` 把公司规则块 4294→2291 B，删掉「等我们的弹窗一分钟/别当超时」那条已被真机证伪的叙事，改单一上游升权叙事；评审 APPROVED + 两条行为塑形规则被删（项目工作范围声明、反「放弃或静默绕行」）→ 补回 `3463e70073`（守卫上限 2400→2500 并注明理由）。
**构建**：b87（preset 修复 + P16 全案 + P6 批①②③）→ b88（+批③修批/复审批）；**b88 用户验收通过**：Settings 只剩 deloitte ✓、pip 弹窗链 ✓。
**沙箱升权遥测缺口（已知项）**：`sandbox_escalation` 累计 0 条——真机走的是**上游** `sandbox_permissions` 授权路（我方 adapter 弹窗是休眠兜底）。选项：接上游 approval 事件、或裁掉该事件，待定。
**engramory 记忆写盘诊断（给 MR session 的意见）**：其 `memoryRoot=~/.dsh/.engramory-memory`；插件本体用原生 `node:fs`（宿主进程，只写模式文件=不受沙箱限），**记忆正文写入由 agent 的文件工具执行 ⇒ 落在 fs 沙箱策略内（工作区外拒）**。四条路：①写入搬到插件侧（注册工具/命令由宿主落盘，推荐、桌面零改动）②记忆降级工作区内 ③上游开可写根（fs/shell 共用封闭 roots，代价不成比例）④每次弹窗升权（体验崩）。建议留 beta 待 ①/② 定夺。
...
**修复状态（13:45）**：已实现 + 评审 **APPROVED**（无 P1）→ `4d4dfab5f5`。
新增回归/边界测试 5 条（staged 陈旧+网络新→放行；两者都旧→拒；网络失败→拒并报原因；非 stale→不重试；纯网络模式不变），
子模块未动，desktop 2365+8skip/market 468/skills 72 全绿，typecheck 0。
评审留下的两个 P2（已记录、不阻发布）：
① **重试依赖子进程自己的 TLS 路径**（undici + Mozilla 根，正是 staged 机制要绕开的那条）；
若真机上 CLI 子进程无法连到 origin，重试会失败 → 安装仍被拒（reason 会写明「受限网络重试失败」而非 stale，可诊断）。
需真机 b91 验证；若真机不行，则落「launcher 安装前重新 stage」那张卡（不依赖子进程 TLS）。
② staged 文件不可用时（缺失/空/超大）已回落到网络，若那份网络字节也 stale，会多发一次同样的网络请求（仅延迟，无信任影响，未加测试）。
...
**P15 状态**：四阶段全落地（`1730e119fc`→`59c5148b2f`，六轮评审 APPROVED，终验全绿），**未构建未发目录**。上 fleet=下下个构建（b82+ 系）；**首战条件**=fleet 有 P15 客户端后 sidebar promote 双钉（0.15.2 保留 stable）——在那之前 promote 仍守「全员升级后」纪律。
**排队**：P11 捆绑 Python（M）→ free-search rebase（可选）。
**挂账**：沙箱档2评估 · sso-gate 浏览器路径主动测 · 0.4.184 fleet 反馈 · P12 正式吸收进 P15（已完成）· 遥测 deferredUpdates 真机观察（P15 客户端上线后）。
**基线**：desktop 2206+7skip · market 466 · catalog e2e PASS · HEAD `59c5148b2f`（fork+backup 已推）。
