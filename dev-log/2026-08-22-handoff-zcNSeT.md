...
| P7 | **技术尾巴** | ①E2E origin 稳态步——**挂起**（单测已覆盖+实机数周稳定，下次动 E2E 顺手）②blob 下载备选——**进行中（2026-08-30 用户拍板做）**：workflow 把签名产物 push 回仓库 artifacts 分支→容器 git 拉取→发布全自动无人工③M1 post-install 校验——**挂起**（低概率+后果非安全+下次升级自愈，等真实案例） | ②进行中 | S |
