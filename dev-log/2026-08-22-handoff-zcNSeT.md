...
**① 权限级别**：锁定构建从 permission presets 配置表（host 组合 base/cordis.patch.yml 的 permission 行）删除 danger-full-access（服务级删除，UI+写路径同步）。**已知残余（用户签收暂不处理）**：预设表管 UI 与正常写路径，直接调 loopback API 裸注入 sandbox 事件理论可绕——执行器（bash-sandbox）钳制调查挂起为待办，触发条件：出现真实绕过尝试或正式推广前复审。
