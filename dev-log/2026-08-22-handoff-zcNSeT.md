...
- **origin 模式切换进行中（2026-08-26）**：用户建公开 GitLab 仓库 https://gitlab.s.dai.deloitte.cn/julu/dsh-desktop-config（Deloitte 内部 CA、raw 匿名可读已验证）；manifest sequence 3 已用演示钥重签（含真实 repository）；待用户上传后：验 raw → 改策略切 origin → 打包
