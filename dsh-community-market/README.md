...
1. **Discover** shows all normalized listings loaded from the selected source. Clicking any card immediately opens the same action dialog, where Desktop checks for a managed installation before falling back to a details view.
...
Clicking a plugin card opens the dialog synchronously and asks the Host whether that exact source/item can use managed installation. A successful preview verifies it against the official npm registry, including identity, repository, integrity, runtime, lifecycle scripts, DSH bundle evidence, and the active profile, then turns the same dialog into an exact confirmation. (Embedded Hosts may additionally inject a tarball-entry verification seam — `MarketTarballEntryVerifier` through the `desktopMarketTarballEntryVerifier` context capability: candidates it claims verify through the Host's signed catalog facts instead of the registry, their installs may run on the Host's controlled tarball target, and the `file:` dependency pin that leaves behind reconciles like an exact-version pin; every candidate the seam declines keeps the registry verification byte-for-byte, as do deployments without the injection.) The Host repeats mutable checks immediately before execution. If managed preview fails, the dialog becomes a details view that explains the failure: transient failures (npm registry jitter, reset connections, timeouts) offer an in-place Retry that re-issues the same preview, while deterministic refusals show the plain failure. The market UI never displays a manual command and never opens a terminal — no `dsh plugin add` text and no Open DSH Terminal button exist in any build or posture; a `dsh plugin add` run later through Desktop's built-in terminal (opened outside the market) still uses Desktop's protected install-recovery boundary, while direct `pnpm` or `npm` commands in that terminal and commands run in an external system terminal do not. After a successful managed profile change, the user can restart immediately with a one-time Desktop action or choose to restart later. The market is a shell around existing DSH capabilities; it does not invent a second plugin format, package manager, profile store, or privileged installer.
...
- The managed installer accepts exact stable npm versions only. It does not install GitHub URLs, mutable ranges or tags, deprecated packages, packages whose target manifest defines `preinstall`, `install`, `postinstall`, or `prepare`, or packages incompatible with the bundled DSH rc.2 or Node.js runtime.
- Provider-supplied command strings, install snippets, and repository install instructions are discarded, never displayed, and never executed; the market UI reconstructs no manual command of its own. For a qualifying dshfind item, the reviewed structured npm method feeds candidate verification only, never display, and `install.cmd` is discarded outright.
- The renderer submits only source/item or receipt identifiers for managed operations. Requesting the one-time restart grant is the only desktop action the market client can invoke.
