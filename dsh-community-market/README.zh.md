...
1. **发现**展示当前来源已经加载并标准化的全部条目。点击任一卡片都会立即打开同一个操作弹窗：Desktop 先检查能否受管安装，不能时再显示详情页。
...
- 目录提供方返回的命令字符串、安装片段和仓库安装指令都会被丢弃，既不会展示，也绝不会执行；市场 UI 不会自行重建任何手动命令。对于符合条件的 dshfind 条目，受审的结构化 npm method 只用于候选验证、绝不用于展示，`install.cmd` 会被直接丢弃。
- 受管操作中，renderer 只提交来源/条目或 receipt 标识。请求一次性重启许可是市场 client 唯一可以调用的桌面操作。
