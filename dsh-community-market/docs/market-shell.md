...
Catalog `install` fields, documentation snippets, provider commands, and arbitrary strings are discarded as execution authority and are never executed or displayed. The market UI reconstructs no manual command of its own: a transient preview failure is answered with an in-place Retry of the same preview, never with terminal guidance. The dshfind adapter specifically discards `install.cmd` and never parses or forwards it. The built-in managed installer rejects GitHub and other repository install targets, ranges, tags, prereleases, deprecated versions, a target manifest containing `preinstall`, `install`, `postinstall`, or `prepare`, packages incompatible with the bundled DSH `0.1.2-rc.1`/Cordis/Node.js runtime, repository mismatches, and packages without official npm SHA-512/tarball and valid DSH bundle evidence.
...
3. Keep stdout, stderr, environment variables, local paths, and command internals out of the renderer; it never receives command text of any kind.
...
When Desktop services are unavailable, browsing stays available while package operations explain that they require DSH Desktop. Managed installation never falls back to ambient `pnpm`, a shell command, a guessed `dsh` executable, or an inactive profile, and the market UI contains no terminal escape hatch: no open-terminal desktop action is served to the market client and no manual command text is displayed. A `dsh plugin add` run later through Desktop's built-in terminal (opened outside the market) receives the same configuration-recovery handoff. Direct `pnpm` or `npm` commands in that terminal and commands run in an external system terminal do not.
...
| Install preview fails from transient network jitter (reset connection, connect timeout, operation deadline) | Bounded failure banner with an in-place Retry that re-issues the same preview; Close remains available | None |
