...
If managed preview fails, the dialog becomes a details view that explains the failure. Transient failures — npm registry jitter, reset connections, timeouts — offer an in-place **Retry** that re-issues exactly the same preview request; deterministic refusals (missing target, integrity or policy refusal) show the plain failure with no retry. The market UI offers no manual terminal path in any build or posture: no **Open DSH Terminal** button and no `dsh plugin add` command display. A `dsh plugin add` launched through Desktop's built-in terminal (opened outside the market, for example from settings or startup recovery) uses the same configuration-recovery record as a Market install. Direct `pnpm` or `npm` commands typed there, and commands run from an external system terminal, are outside this recovery boundary. A manual install creates no Market receipt and therefore grants no Market uninstall authority.
...
- The renderer never receives filesystem, process, environment, or package-manager authority. Package changes go through `desktopPnpm.runPlugin()` with fixed argument construction and the active profile's absolute directory. The renderer receives no command-shaped values at all.

The receipt records the profile, exact npm identity, integrity, DSH bundle patch, catalog provenance, display name, and installation time. It is local evidence that this Market completed and verified a managed install; it is not a provider credential and must not depend on the source remaining registered. In deployments that install through the signed company catalog, receipts are version 2 and additionally record the signed manifest sequence and trust-root key that allowed the install, a SHA-256 measurement of the installed package tree taken right after installation, and `resolved`/`decided` evidence fields using the dsh-community-fabric RFC 0004 evidence vocabulary. A receipt of any version is a cache hint and an uninstall reconciliation credential only: install permission is always decided again from the signed company manifest, registry metadata, and the post-install measurement, never from a stored receipt. Legacy version-1 receipts stay readable for uninstall reconciliation but carry no signed evidence.

If Desktop package capabilities are unavailable, browsing still works while install, uninstall, disable, and enable return an unavailable state. The managed path never falls back to an ambient `pnpm`, shell, guessed executable, repository command, or inactive profile, and the market UI opens no terminal and displays no manual command — a transient preview failure is answered with Retry, not a detour around the managed path. Only a `dsh plugin add` run later through Desktop's built-in terminal receives the Desktop recovery handoff; other terminal commands do not.
