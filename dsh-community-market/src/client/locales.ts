export const zh = {
  ...
  installCommand: '安装命令',
  manualBuildApprovalRequired: '此插件需要允许依赖执行本机构建脚本。插件市场不会代你授予这项代码执行权限，请在终端中检查 pnpm 列出的依赖后自行决定。',
  manualBuildPolicyUnverified: '当前来源没有提供经过审核的构建策略。为避免图形界面意外执行第三方构建脚本，插件市场不提供自动安装。',
  buildApprovalDetected: 'npm 最新版本声明了安装或构建脚本，桌面端已在修改 Profile 前停止自动安装。请检查下方命令并在终端中自行决定是否授权。',
  buildPolicyChanged: 'npm 最新版本与目录审核过的版本不一致，桌面端无法确认新的依赖是否需要构建许可，因此已在修改 Profile 前停止自动安装。请刷新来源或检查下方命令。',
  ...
  mutableNpmWarning: '这条命令未固定 npm 版本，执行时会解析当时的最新版；安装前请再次核对包名和版本。',
  ...
  installableBody: '这里仅显示当前来源中经过审核、确认指定版本无需构建脚本许可的 npm 插件。安装前仍会复查 npm 最新稳定版、审核版本和 DSH bundle。',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  manualBuildApprovalRequired: 'This plugin requires permission for dependencies to run local build scripts. Plugin Market will not grant that code-execution permission for you; review the dependencies listed by pnpm in the terminal and decide explicitly.',
  manualBuildPolicyUnverified: 'The current source does not provide a reviewed build policy. Plugin Market disables automatic installation to avoid unexpectedly running third-party build scripts from the graphical interface.',
  buildApprovalDetected: 'The latest npm release declares install or build scripts, so Desktop stopped automatic installation before changing the Profile. Review the command below and decide in the terminal whether to grant permission.',
  buildPolicyChanged: 'The npm latest version no longer matches the reviewed catalog release. Desktop cannot confirm whether its new dependencies need build approval, so automatic installation stopped before changing the Profile. Refresh the source or review the command below.',
  ...
  mutableNpmWarning: 'This command does not pin an npm version and resolves the latest release at execution time. Recheck the package name and version before installing.',
  ...
  installableBody: 'This view shows only npm releases reviewed as requiring no build-script approval. Desktop still rechecks npm latest against the reviewed version and verifies the DSH bundle before installation.',
