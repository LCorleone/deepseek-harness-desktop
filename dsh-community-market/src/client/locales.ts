export const zh = {
  ...
  installedBody: '这里显示当前配置中的插件。通过插件市场安装的插件可以卸载；通过其他方式安装的插件只能禁用。',
  install: '安装',
  uninstall: '卸载',
  disable: '禁用',
  ...
  disabledRestartRequired: '此插件已设为禁用。',
  ...
  cancel: '取消',
  confirmAdd: '添加来源',
  sourceNotice: '可以添加多个来源，但每次只使用一个。远程来源只能提供目录数据，不能在本机执行操作。',
  sourceError: '来源操作失败',
...
export const en: Record<MarketLocaleKey, string> = {
  ...
  installedBody: 'Plugins in the active profile appear here. Plugins installed by Plugin Market can be uninstalled; plugins installed another way can only be disabled.',
  install: 'Install',
  uninstall: 'Uninstall',
  disable: 'Disable',
  ...
  disabledRestartRequired: 'This plugin is set to disabled.',
  ...
  sourceNotice: 'You can add multiple sources, but only one is used at a time. Remote sources provide catalog data and cannot perform local actions.',
