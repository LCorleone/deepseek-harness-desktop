...
Windows 打包应用还自带官方 embeddable CPython 3.12：启动时把 `python`、`python3` 与 `py` 三个别名前置到同一 Electron 进程的 `PATH`（同样不改系统 `PATH`，macOS 与 Linux 不捆绑）。pip 与 virtualenv 已随树预装，装包应落在工作区 `.venv`（`python -m virtualenv .venv`）而不是全局 site-packages；捆绑树按逐文件 sha256 digest 清单在每次启用前整树校验，校验失败时该命令面 fail-closed 禁用，不影响应用其余功能。未打包的 Windows 开发运行复用系统 `PATH` 上的 `python`。

`desktop-pnpm` Host row 会提供 `ctx.desktopPnpm`，用于针对不可变激活 profile 执行受管 package operation。`run(args, signal?)` 会在激活 profile 目录中直接执行内置 pnpm；它是低层 operation，不承诺 DSH profile 初始化、调用方相对 source 锚定或 bundle reconcile。`runPlugin(args, invokingDir, signal?)` 会为 remove、update 和 dependency repair 启动内置 `dsh plugin --profile <active>`，但拒绝 `add`。插件安装使用 `installPlugin(request)`：它把精确 `name@version` 目标与 receipt 绑定，spawn 前快照 profile，并在完成前封存或恢复快照。锁定构建上，Host 注入的通道（P7 2c，`src/company-market-install.ts`）会在请求对应的签名 company-manifest 条目走 tarball 通道时，把该请求分流到受控 tarball 目标：先暂存签名 tarball（下载、sha512 校验、受控暂存路径），再以唯一可构造的 `file:<staged>` 目标、同一 receipt 安装，装后做 bundle 与签名 tree 复验并支持回滚——其余请求逐字节保持 registry 路径不变。上游 CLI 仍拥有 pnpm profile working directory 和 bundle reconcile 语义。

两个方法都会返回实时 stdout 与 stderr stream、在完整 process tree 退出后才 settle 的 `done` promise，以及 `cancel()`。每个 generation 同时最多运行一个 operation。Service 使用普通 DSH subprocess provider、准确的已打包 JavaScript entry、无 shell argv，以及只属于 child 的 DSH home、随应用捆绑的 Node 运行时、CI 与 native-module ABI 值。公开 runtime path 仍不会暴露 `node` 或 `dsh`；其中私有 helper 与 npm ABI 变量只存在于 package-manager subprocess tree 内。Launcher 不会修改系统 `PATH`、shell 启动文件、profile 配置或 `.env` 文档。
...
嵌入策略带 `usageReport` 的构建（release 变体）会为公司 MySQL 的 `dsh_model_call_events` 表按每次模型调用记录一条遥测行：已认证的 SSO 邮箱（无会话时为空 —— 上报从不依赖 SSO）、provider、model 与托管网关 base URL；互斥的 token 桶（input、cache read、cache write、output、仅作为 output 子集的 reasoning，以及四桶合计的 total）；tokens per second、首 token 延迟与步时延；会话 ID 及 turn/step 序号；产品版本；事件时间。不会从会话中读取任何提示词、回复、思维链文本、工具数据或错误文本 —— 行的内容就是这组列。行先进入有界的 5,000 条内存队列（满时丢最旧并计数），按 50 条一批或每 10 秒双触发落库，首次落库才懒建立 MySQL 连接，写失败按封顶 60 秒的指数退避重试，退出时尽力排空。低权限仅 INSERT 的凭据以混淆 blob 分发（混淆不是加密），`DSH_REPORT_DB_*` 环境覆盖仅对未打包运行生效，模块日志只携带计数器与连接状态 —— 绝不携带目标地址或凭据。dev 与未锁定构建完全不接线上报器。
...
`yarn package:dir` 为当前宿主平台创建未封装目录。如果应用归档缺少 desktop 更新与终端模块、DSH CLI bootstrap、内置 pnpm 入口或物理 deployment package，packaged-runtime gate 会拒绝该产物。Electron Builder 会把根 manifest、desktop runtime 与完整依赖树输出到 `app.asar.unpacked`；Host profile boot 与 CLI bootstrap 都会使用这棵物理树，因此 DSH profile fallback 的符号链接不会指向虚拟 ASAR 目录；捆绑 Node 的 `dsh`/pnpm 入口与 native 预编译集（node-pty、koffi、sharp、ripgrep）也保持物理存在，因为普通 Node 子进程无法读取 ASAR 归档内部。只有生成的图标资产留在 `app.asar` 内部：Electron 主进程通过透明的虚拟路径读取它们。`build/app-icon.png` 保持为未经修改的 iOS Default 源图，并继续作为 Linux 应用图标。构建过程会派生 Windows 专用的 `build/app-icon.ico`，为常用高 DPI 档位提供精确帧，在小尺寸使用简化的矢量样式，并让 256 像素以下的帧采用兼容性更好的 DIB payload；应用程序、NSIS 安装器与卸载器都会使用该图标。构建也会运行 `scripts/generate-mac-app-icon.mjs`，把源图缩放为 824 × 824 像素并居中放入透明的 1024 × 1024 画布；macOS 打包与运行中的 Dock 都使用生成的 `build/app-icon-mac.png`。`build/tray-icon.svg` 是品牌蓝托盘源文件：构建过程会派生由 macOS 系统自动着色的模板图，以及固定品牌蓝的 Windows 与 Linux 托盘图。

打包产物附带完整的 Electron fuse 集：`runAsNode: false`、`enableCookieEncryption: true`、`enableNodeOptionsEnvironmentVariable: false`、`enableNodeCliInspectArguments: false`、`enableEmbeddedAsarIntegrityValidation: true`、`onlyLoadAppFromAsar: true`、`loadBrowserProcessSpecificV8Snapshot: false` 与 `grantFileProtocolExtraPrivileges: true`——唯一一条「放宽」方向的 fuse：打包态 native-ui 窗口（sso-gate/recovery/profile-create/agent-browser）均为自有 file:// 模块化 UI，无此特权时 Chromium 把每个打包 file:// 文档跑成不透明（null）源、ES module 被 CORS 直接拒载（真机恢复窗黑屏与 agent-browser 故障即此；开发态二进制 fuse 默认不同，放过了冒烟）。豁免有界：这些页面带严格 `default-src 'none'` CSP、不加载任何不可信 file:// 内容，guest 网页运行于独立 webContents/进程、不受该 fuse 影响；vite 同源子树守门继续把所有 native-ui 文档锁在自身目录内作为纵深。启动时 Electron 会把 `app.asar` 的 header 哈希与构建器嵌入平台二进制的值（Windows 的 PE 资源，或 macOS 中绑定进代码签名的 `Info.plist` 条目）进行比对：任何对归档结构的修改——新增、删除、重命名、改变条目大小，或在归档与物理镜像之间移动文件——都会在应用代码运行前失败，而 `onlyLoadAppFromAsar` 拒绝从松散目录加载应用。平台范围：Electron 仅在 macOS（Electron 16+）与 Windows（Electron 30+）目标上强制内嵌 ASAR 完整性校验——在 Linux 产物上该 fuse 位仍然写入二进制，但 Linux 运行时不做任何校验，因此 Linux 归档加固保持 advisory，完全依赖安装目录的权限控制。该加固是 advisory（成本项）而非强制边界：在没有 Authenticode 签名（Windows）或 Developer ID 签名（macOS）的情况下，有决心的攻击者可以把 fuse 位或嵌入哈希从无签名二进制中改回；有意保留在 `app.asar.unpacked` 中的文件字节也不在本检查范围内。这些 fuse 提高的是脚本化篡改与不知情工具的攻击成本；具有约束力的保证仍需签名发布，这一项仍是“已知限制”中记录的发布门禁。

### Electron fuse 全集、发布清单与平台门禁矩阵

Electron Builder 不支持按模式区分的 fuse 配置，而 fuse 只作用于打包后的二进制，因此一份配置同时服务开发与发布：每一个打包产物——包括本地 `--dir` 冒烟构建——都是发布候选。`yarn dev` 运行的是未打包的 Electron 发行版，`--inspect` 调试、`NODE_OPTIONS` 与 Electron-as-Node 在开发态照常可用；打包产物则在 Electron 运行时忽略 `NODE_OPTIONS`/`NODE_EXTRA_CA_CERTS`，并拒绝 `--inspect`、`--inspect-brk` 与 `SIGUSR1` 激活 inspector，因此打包态问题通过日志、诊断导出或未打包 dev 运行来定位。托管子进程使用独立的捆绑 Node 二进制（P3-1），保持自己的运行时语义。`afterPack` 钩子断言配置的 fuse 集合与发布态要求完全一致——多余、拼写错误或缺失的 fuse 键都会让构建失败，而不是被静默忽略——随后从打包后的应用二进制中读回 fuse 位图，因此打包回归或打包后的翻转都会在签名前让构建失败。
