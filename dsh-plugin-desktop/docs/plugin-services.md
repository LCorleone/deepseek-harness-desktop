...
On locked builds the Electron host injects one optional diversion into this boundary (P7 2c): when the request carries no controlled tarball descriptor and the signed company-manifest entry for its exact `(packageName, packageVersion)` is published on the tarball channel, the Host channel (`src/company-market-install.ts`, provided as `desktopCompanyMarketTarballInstall`) takes the request over — it downloads and stages the signed tarball into the profile's controlled staging area, re-runs the install through the one constructible `marketTarball` descriptor with the same receipt, re-verifies the installed bundle and the signed tree digest, and rolls the profile back on divergence. The synthesized handle settles nonzero with the readable reason on its stderr when any step refuses. Every other request — and every deployment without the injection — keeps the registry path above byte-for-byte, and no user-facing argument surface can produce a diversion because the descriptor remains in-process only.
