...
Compatibility and extended modes report the same 36-pixel top reservation and drag band on macOS and Windows; they exclude 80 pixels on the left for macOS traffic lights or 138 pixels on the right for Windows caption controls. Desktop already shifts the complete official frame below this reservation, so ordinary upstream-slot occupants must not add it again. Extended differs only by continuing the frame material behind the official sidebar. Linux compatibility keeps its ordinary native frame and therefore reports zero insets and a zero-height drag region. Advanced macOS and Windows also report a 36-pixel top content inset and drag band; macOS excludes 80 pixels on the left and Windows excludes 138 pixels on the right for native controls.
