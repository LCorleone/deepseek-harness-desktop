...
describe('advanced desktop layout', () => {
  it('owns native caption geometry with one fixed macOS drag strip above page content', () => {
    ...
    try {
      ...
      expect(css).not.toContain('.dshDesktopSidebarSurface::before')
      expect(css).toMatch(new RegExp(`\\.dshDesktopMacCaptionRow \\{[^}]*position: absolute;[^}]*z-index: 40;[^}]*top: 0;[^}]*right: 0;[^}]*left: ${MACOS_TRAFFIC_LIGHT_SAFE_WIDTH}px;[^}]*height: ${ADVANCED_MACOS_DRAG_REGION_HEIGHT}px;[^}]*-webkit-app-region: drag;`))
