...
describe('extended desktop layout', () => {
  it('reserves a visible command bar and rounds the inner corner of the inverted-L glass frame', () => {
    ...
    try {
      ...
      expect(css).toMatch(/> :first-child \{[^}]*background: var\(--dsh-desktop-extended-glass-fill\)/)
      ...
      expect(css).toMatch(/\.dshDesktopExtendedIdentity \{[^}]*left: 50%;[^}]*transform: translateX\(-50%\);/)
      expect(css).toMatch(/\.dshDesktopExtendedActions \{[^}]*-webkit-app-region: no-drag;/)
      expect(css).toContain('[data-platform="darwin"] .dshDesktopExtendedActions { margin-left: auto; }')
      expect(css).toContain('[data-platform="win32"] .dshDesktopExtendedActions { margin-right: auto; }')
