// Deloitte co-brand wordmark for the generic sidebar brand-name slot:
// "Deloitte DeepSeek" as live text plus the upstream HARNESS badge. The
// badge plate and knocked-out letterforms are copied verbatim from the
...
const VIEW_WIDTH = BADGE_X + BADGE_WIDTH + 2 - TEXT_X
const BADGE_TRANSLATE_X = Math.round((BADGE_X - BADGE_NATIVE_X) * 1e3) / 1e3

/**
 * Render the co-branded name artwork without the independently slotted mark.
 * @returns the wordmark svg (aria-hidden decorative brand art; the sidebar
 * button carries the accessible label).
 */
export function DeloitteBrandName() {
  return (
    <svg
      width={VIEW_WIDTH}
    ...
    >
      ...
      >
        ...
        <tspan dx={WORD_GAP} fill="currentColor">DeepSeek</tspan>
      </text>
      <g transform={`translate(${BADGE_TRANSLATE_X} 0)`}>
        <rect x="129.348" y="5.5" width="52" height="14" rx="2" fill="currentColor"/>
        <g clipPath="url(#deloitte-brand-badge-clip)">
        ...
        <defs>
          <clipPath id="deloitte-brand-badge-clip">
