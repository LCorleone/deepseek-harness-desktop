...
export class WindowsAgentPresets extends AgentPresets {
  ...
  override async resolve(id?: string): Promise<AgentPreset> {
    ...
    throw unknownPresetError(id, (await this.list()).map(candidate => candidate.id))
  }
  ...
  override async copy(from: string, id: string, name?: string): Promise<void> {
    if (id === WINDOWS_UNSUPPORTED_PRESET) throw presetExistsError(id)
    await super.copy(from, id, name)
  }
}

/** Stable `agent-preset/not-found` rejection used by the Windows roster. */
function unknownPresetError(id: string, available: readonly string[]): RemoteError {
...
/** Stable `agent-preset/invalid` rejection used by the Windows roster. */
