...
const COPY: Record<DesktopLocale, DesktopSetupWizardCopy> = {
  en: {
    ...
    networkExposureBody: 'Desktop currently keeps access on this computer. Local-network access will become available after trusted HTTPS support is ready.',
    loopback: 'This computer only',
    loopbackBody: 'Listen on loopback addresses only.',
    lan: 'Local network',
    lanBody: 'Temporarily unavailable. Trusted HTTPS is required before other devices on the LAN can connect.',
  ...
  zh: {
    ...
    networkExposureBody: 'DSH Desktop 目前只允许本机访问；可信 HTTPS 支持完成后才会开放局域网访问。',
    loopback: '仅这台电脑',
    loopbackBody: '只监听本机回环地址。',
    lan: '局域网',
    lanBody: '暂不可用。必须先提供可信 HTTPS，才会允许局域网中的其他设备连接。',
