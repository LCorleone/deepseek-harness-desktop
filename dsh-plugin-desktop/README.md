...
The Windows package additionally bundles the official embeddable CPython 3.12: at startup the launcher prepends the three interchangeable aliases `python`, `python3`, and `py` to the same Electron process `PATH` (again without touching the system `PATH`; macOS and Linux bundle no Python). pip and virtualenv ship preinstalled inside the tree, package installs should land in a workspace `.venv` (`python -m virtualenv .venv`) rather than the global site-packages, and the bundled tree is verified in full against a per-file sha256 digest manifest before each use — a failed verification disables that command surface fail-closed without affecting the rest of the application. An unpackaged Windows development run instead reuses the `python` command already on the system `PATH`.

The `desktop-pnpm` Host row provides `ctx.desktopPnpm` for managed package operations against the immutable active profile. `run(args, signal?)` executes packaged pnpm directly in the active profile directory; it is a low-level operation and does not promise DSH profile initialization, caller-relative source anchoring, or bundle reconciliation. `runPlugin(args, invokingDir, signal?)` starts packaged `dsh plugin --profile <active>` for remove, update, and dependency repair, but rejects `add`. Plugin installation uses `installPlugin(request)`, which binds one exact `name@version` target to a receipt, snapshots the profile before spawning, and seals or restores the snapshot before completion. On locked builds a Host-injected channel (P7 2c, `src/company-market-install.ts`) diverts one such request to the controlled tarball target when its signed company-manifest entry is published on the tarball channel: the request then stages the signed tarball (download, sha512 verify, controlled staging path) and installs it as the one constructible `file:<staged>` target with the same receipt, post-install bundle and signed-tree re-verification, and rollback — every other request keeps the registry path byte-for-byte. The upstream CLI remains authoritative for pnpm profile working-directory and bundle-reconciliation semantics.

Both methods return live stdout and stderr streams, a `done` promise that settles after the complete process tree exits, and `cancel()`. One operation may run per generation. The service uses the ordinary DSH subprocess provider, exact packaged JavaScript entries, shell-free argv, and child-scoped DSH home, the Node runtime bundled beside the application, CI, and native-module ABI values. The public runtime path still does not expose `node` or `dsh`; its private helper and the npm ABI variables exist only inside package-manager subprocess trees. The launcher does not modify the system `PATH`, shell startup files, profile configuration, or `.env` documents.
...
Builds whose embedded policy sets `usageReport` (the release variant) record one telemetry row per model call into the company MySQL `dsh_model_call_events` table: the authenticated SSO email (empty when no session exists — reporting never depends on SSO), provider, model, and managed-gateway base URL; the disjoint token buckets (input, cache read, cache write, output, reasoning as an output-only subset, and the four-bucket total); tokens per second, time-to-first-token, and step latency; the session id with turn and step numbers; the product version; and the event time. No prompt, response, reasoning text, tool data, or error text is ever read from the conversation — the row is exactly that column set. Rows wait in a bounded 5,000-entry memory queue (drop-oldest with a counter) that flushes 50 rows at a time or every ten seconds, opens the MySQL connection lazily on the first flush, retries failed writes with a doubling backoff capped at sixty seconds, and drains best-effort during quit. The low-privilege INSERT-only credential ships as an obfuscated blob (obfuscation is not encryption), `DSH_REPORT_DB_*` environment overrides apply to unpackaged runs only, and this module's log lines carry counters and connection status only — never the destination or credential. Dev and unlocked builds keep the reporter completely unwired.
...
`yarn package:dir` creates an unpacked directory for the current host platform. The packaged-runtime gate rejects an application archive that omits the desktop update and terminal modules, the DSH CLI bootstrap, the bundled pnpm entry, or the physical deployment package. Electron Builder emits the root manifest, desktop runtime, and complete dependency tree under `app.asar.unpacked`; both Host profile boot and the CLI bootstrap use this physical tree so DSH profile-fallback symlinks never target a virtual ASAR directory, and the bundled-Node `dsh`/pnpm entries plus the native prebuild trees (node-pty, koffi, sharp, ripgrep) stay physical because a plain Node child cannot read inside an ASAR archive. Only the generated icon assets stay inside `app.asar`: the Electron main process reads them through transparent virtual paths. `build/app-icon.png` remains the unmodified iOS Default source and the Linux application icon. The build derives a Windows-specific `build/app-icon.ico` with exact high-DPI frames, a simplified vector treatment at small sizes, and compatibility DIB payloads below 256 pixels; the application executable, NSIS installer, and uninstaller all use that icon. It also runs `scripts/generate-mac-app-icon.mjs` to center the source artwork at 824 by 824 pixels on a transparent 1024 by 1024 canvas; macOS packaging and the live Dock both use the generated `build/app-icon-mac.png`. `build/tray-icon.svg` is the brand-blue tray source: the build derives a macOS template image that the system colors automatically and fixed brand-blue Windows and Linux tray images.

Packaged applications ship the complete Electron fuse set: `runAsNode: false`, `enableCookieEncryption: true`, `enableNodeOptionsEnvironmentVariable: false`, `enableNodeCliInspectArguments: false`, `enableEmbeddedAsarIntegrityValidation: true`, `onlyLoadAppFromAsar: true`, `loadBrowserProcessSpecificV8Snapshot: false`, and `grantFileProtocolExtraPrivileges: true` — the one fuse that relaxes rather than hardens, granted because every packaged native-ui window (sso-gate, recovery, profile-create, agent-browser) is a self-owned module bundle loaded over `file://`: without the privilege Chromium runs each packaged `file://` document as an opaque (`null`) origin whose ES modules CORS refuses outright, which blanked the recovery window and broke the agent browser on real installs while the dev binary's different fuse default let smoke pass. The grant is bounded: those pages ship strict `default-src 'none'` CSPs, load no untrusted `file://` content, and guest web pages run in their own webContents/processes that this fuse does not reach; the vite file-origin subtree guard additionally keeps every native-ui document inside its own directory as defense-in-depth. At startup Electron compares the `app.asar` header hash against the value the builder embeds in the platform binary (a Windows PE resource, or the macOS `Info.plist` bound into the code signature), so editing the archive structure — adding, removing, renaming, or resizing entries, or flipping a file between the archive and the physical mirror — fails the check before application code runs, and `onlyLoadAppFromAsar` rejects loose-file application roots. Platform scope: Electron only enforces the embedded ASAR-integrity check on macOS (Electron 16+) and Windows (Electron 30+) targets — on the Linux artifact the fuse value stays staged in the binary but the Linux runtime performs no validation, so Linux archive hardening remains advisory and rests entirely on installation-location permissions. This hardening is advisory, not a boundary: without an Authenticode signature (Windows) or a Developer ID signature (macOS), a determined actor can patch the fuse wire or the embedded hash back out of the unsigned binary, and the bytes of files that intentionally stay unpacked in `app.asar.unpacked` are not covered by this check at all. The fuses raise the cost of drive-by tampering with scripts and unsuspecting tooling; a binding guarantee requires signed releases, which remain a release gate documented under Known Limitations.

### Electron fuse set, release checklist, and platform gate matrix

Electron Builder has no per-mode fuse profiles, and fuses apply only to packaged binaries, so one map serves both development and release: every packaged artifact — including a local `--dir` smoke build — is a release candidate. `yarn dev` runs the unpackaged Electron distribution, where `--inspect` debugging, `NODE_OPTIONS`, and Electron-as-Node keep working; packaged builds ignore `NODE_OPTIONS`/`NODE_EXTRA_CA_CERTS` in the Electron runtime and reject `--inspect`, `--inspect-brk`, and `SIGUSR1` inspector activation, so packaged problems are diagnosed through logs, the diagnostics export, or an unpackaged dev run. Managed subprocesses use the separate bundled Node binary (P3-1) and keep their own runtime semantics. The `afterPack` hook asserts that the staged fuse map equals the required release posture exactly — an extra, mistyped, or missing fuse key fails the build instead of being silently ignored — and then reads the fuse wire back out of the packaged application binary, so a packaging regression or a post-pack flip fails the build before signing.

The same hook runs the company release checklist: the release policy asset stays `locked: true`, the update-channel trust roots are either pinned keys or the explicitly marked development placeholder, and the update-manifest anti-rollback state file (`userData/updates/manifest-sequence.json`, P3-3) stays wired into both Electron call sites — the scheduled version check and the confirmed installer download.

Platform gate matrix (gaps are recorded for the follow-up hardening card, not implemented here):

| Gate | macOS | Windows |
| --- | --- | --- |
| Code signature | Developer ID, asserted by `release-preflight` and re-verified with `codesign --verify --deep --strict` | None — `dist:win` pins `signExecutable=false` and strips certificate variables |
| Notarization / publisher | Notarized and stapled; `spctl --assess` plus `stapler validate` gate the DMG | Unsigned NSIS/portable artifacts; Authenticode and SmartScreen reputation remain open |
| Runtime hardening | `hardenedRuntime: true` with notarization | Fuse set only; no Authenticode anchor for the advisory fuses |
| Installer posture | Drag-install from a signed DMG | NSIS per-user with assisted elevation (`perMachine: false`) |

Recorded gaps for the follow-up card: Windows has no release preflight at all (the current one is macOS-only) and no post-signing signature validation; machine-wide (`perMachine`) installation and its ACL posture are unverified; the unsigned macOS smoke build flips fuse bytes without re-signing, which invalidates the Apple Silicon ad-hoc signature — its gate verifies files only and never launches the app, and restoring a launchable ad-hoc signature (`resetAdHocDarwinSignature` or a smoke re-sign step) is undecided.

Conditional appendix (documented, deliberately unscheduled): if a company budget or IT partnership later provides an Authenticode certificate and Windows infrastructure, the client needs zero changes to upgrade from advisory to enforced semantics — set `win.signExecutable`/signing variables instead of the stripped local ones, flip NSIS to `perMachine`, and the P3-1/P3-2 fuse wire and embedded ASAR hash become anchored by signature verification. The macOS column already demonstrates this shape end to end.
...
- On Windows, the ambient `pnpm` command and lifecycle Node helper are `.cmd` shims. `desktopPnpm.run()`, `runPlugin()`, and `installPlugin()` avoid shell lookup for the manager process by launching exact packaged entries, while upstream `dsh plugin`, PowerShell, and Command Prompt can resolve the ambient shim through a command interpreter. A third-party plugin that calls Node `spawn('pnpm', { shell: false })`, or a lifecycle script that directly executes its `.cmd` `npm_node_execpath` with `shell: false`, remains non-portable and should use the managed service or a shell-aware launch path.
