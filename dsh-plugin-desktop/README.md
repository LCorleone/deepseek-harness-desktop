...
`yarn package:dir` creates an unpacked directory for the current host platform. The packaged-runtime gate rejects an application archive that omits desktop runtime modules, the DSH CLI bootstrap, the bundled pnpm entry, or a required native binary. Ordinary JavaScript and resources stay in `app.asar`; only native modules, external executables, agent presets, and the diagnostics Worker are emitted under `app.asar.unpacked`. The packaged profile resolver falls back to the installation dependency tree inside ASAR without materializing or copying that tree into each profile. `build/app-icon.png` remains the unmodified iOS Default source and the Windows/Linux application icon. The build runs `scripts/generate-mac-app-icon.mjs` to center that artwork at 824 by 824 pixels on a transparent 1024 by 1024 canvas; macOS packaging and the live Dock both use the generated `build/app-icon-mac.png`. `build/tray-icon.svg` is the brand-blue tray source: the build derives a macOS template image that the system colors automatically and fixed brand-blue Windows and Linux tray images.
...
Run `yarn dist:linux` on a native glibc Linux x64 or arm64 host. The command runs the Linux package gate, validates the installed x64 and arm64 Landlock, Sharp, Koffi, ripgrep, Node-API, and node-pty binaries, then emits and verifies four artifacts under `dsh-plugin-desktop/dist/linux/`: x86-64 and arm64 AppImages plus amd64 and arm64 DEBs. The verifier checks each AppImage and staging executable's ELF architecture, each DEB's archive header, package metadata and installed ASAR paths, and the shared packaged-runtime gate checks the architecture-specific native files before Electron Builder seals an artifact.

The AppImage is the installation-speed-first path: make it executable and launch it directly, with no extraction or package-manager install step. It uses the static AppImage runtime and Zstandard compression for fast startup without a FUSE 2 dependency. The DEB uses Zstandard compression and installs `dsh-desktop` through the system package manager when desktop-menu registration and managed uninstall are preferred. CI builds both formats on Ubuntu and uploads the four compressed files without recompressing them.
