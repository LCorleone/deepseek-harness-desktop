...
Extended mode preserves the official upstream `ui-layout`, sidebar, conversation, and details occupants as a compatibility-enhanced presentation. Desktop adds a fixed 36 CSS-pixel command bar above that intact frame and shifts the entire official surface below it, so upstream content never needs element-specific title-bar safe-area compensation. The command bar and complete official left sidebar reveal one non-stacking material layer, forming a continuous inverted-L glass surface. The main conversation surface sits inside that L and clips through a 14-pixel rounded inner corner.
