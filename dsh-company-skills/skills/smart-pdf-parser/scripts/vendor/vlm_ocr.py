...
`vlm-image` skill (scripts/vlm_ocr.py) into smart-pdf-parser because the skill
executor stages each skill in its own private directory and the source tree's
../../vlm-image sibling lookup cannot resolve there. It will NOT track future
edits to the `vlm-image` skill — re-vendor (and re-review) whenever that skill
changes.
"""
