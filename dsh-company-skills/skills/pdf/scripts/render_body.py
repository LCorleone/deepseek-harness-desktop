...
# ── Font registration with CJK support ──────────────────────────────────────────
import re
import subprocess
...
def discover_cjk_font() -> tuple[str, str] | None:
    """
    Discover a CJK font using fontconfig.
    Returns (font_name, font_path) or None.

    Priority: user env > fontconfig > common paths
    """
    env_path = os.environ.get("PDF_CJK_FONT")
    if env_path and os.path.exists(env_path):
        return ("CJKCustom", env_path)
        ...
        if result.returncode == 0:
            for line in result.stdout.strip().split("\n"):
                ...
                if path and os.path.exists(path):
                    if path.endswith((".ttf", ".ttc", ".TTF", ".TTC")):
                        return ("CJKFont", path)
...
def register_fonts(tokens: dict):
    ...
    try:
        ...
        tokens["font_display_rl"] = name
        tokens["font_body_rl"] = name
        tokens["font_body_b_rl"] = name
    except Exception as e:
        print(f"Warning: CJK font registration failed: {e}", file=sys.stderr)
...
def _image_from_bytes(
    png_bytes: bytes, usable_w: float, max_frac: float = 0.88
) -> RLImage:
...
def _render_chart_png(item: dict, accent: str, dpi: int = 150) -> bytes | None:
    ...
    try:
        ...
        if title_text:
            ax.set_title(
                title_text, fontsize=10, pad=8, color="#333333", fontweight="bold"
            )
...
def _render_flowchart_png(
    item: dict, accent: str, dark: str, muted: str, dpi: int = 130
) -> bytes | None:
    """
    ...
    try:
        ...
        for nid, (cx, cy) in pos.items():
            ...
            if shape in ("oval", "terminal"):
                ...
                ax.text(
                    cx, cy, label, ha="center", va="center", fontsize=8, color=dark_rgb
                )
...
def _add_callout(story: list, item: dict, ctx: dict):
    ...
    story.append(
        CalloutBox(item["text"], ctx["styles"]["callout"], ctx["acc"], ctx["acc_lt"])
    )
    story.append(Spacer(1, 8))
...
def _add_code(story: list, item: dict, ctx: dict):
    ...
    font_name = ctx["tokens"]["font_body_rl"] if contains_cjk(code_text) else "Courier"
    code_style = ParagraphStyle(
        "Code",
        fontName=font_name,
        fontSize=8.5,
        leading=12.5,
        textColor=HexColor(ctx["tokens"]["body_text"]),
    )
