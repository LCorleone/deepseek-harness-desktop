...
def parse_markdown(text: str) -> list:
    ...
    while i < len(lines):
        ...
        if m:
            ...
            para_buf = []
            level = len(m.group(1))
            htype = {1: "h1", 2: "h2", 3: "h3", 4: "h4"}.get(level, "h4")
...
def parse_file(input_path: str) -> tuple[list, list]:
    ...
    if ext == ".pdf":
        ...
        return blocks, [
            "PDF text extraction is best-effort — review content.json before rendering"
        ]
...
def main():
    ...
    parser.add_argument(
        "--input", required=True, help="Input file (.md, .txt, .pdf, .json)"
    )
    parser.add_argument(
        "--out", default="content.json", help="Output content.json path"
    )
    args = parser.parse_args()

    if not os.path.exists(args.input):
        print(
            json.dumps({"status": "error", "error": f"File not found: {args.input}"}),
            file=sys.stderr,
        )
        sys.exit(1)
