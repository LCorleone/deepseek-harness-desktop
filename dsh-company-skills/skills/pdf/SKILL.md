---
name: pdf
description: "Design-quality PDF skill for when visual quality and design identity matter. CREATE from scratch: 'make a PDF', 'generate a report', 'create a resume', 'beautiful PDF', 'professional/client-ready document'. FILL form fields: 'fill in the form', 'what fields does this PDF have'. REFORMAT an existing doc: 'apply our style', 'convert Markdown/text to PDF', 're-style this PDF'. Token-based design system: color, typography, spacing derived from document type; output is print-ready."
license: MIT
---
...
> **External Tools — three-stage rule** (`playwright` + Chromium are not bundled with this skill). `render_cover.js` — the cover step of the CREATE/REFORMAT pipeline — needs them:
> 1. **Check first:** `npx playwright --version` (probes PATH and this skill's `tools/` dir; `make.sh check` reports the same).
