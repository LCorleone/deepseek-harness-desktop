...
2. **Plan slide mapping**: For each content section, choose a template slide.
   ...
   **Vary structure using the template's visual vocabulary.** Monotonous decks fail — seek variety. But get variety through **structure** (which slides, what each covers, dropping unneeded slots), not through new styling. Reuse the layout patterns the template already demonstrates (multi-column, image+text, full-bleed, quote, divider, stat, icon-grid); don't invent new visual elements or restyle. When mapping content:

   - Survey which layouts the template provides and reuse them.
   - Pick a different layout for each section, matched to the content (key points → bullet, team → multi-column, testimonials → quote).
   - **Avoid:** the same text-heavy layout on every slide; and **avoid** introducing new colors/images/icons the template doesn't use.

   See the Preservation Contract above — structure is yours, styling is the template's.
