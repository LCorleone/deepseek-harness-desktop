---
name: ppt-designer
description: "Create and replicate presentations: generate a deck from a topic/document/outline, replicate one from images or PDF, or build Deloitte-styled decks from the built-in PPTD template (local .pptx export, font embedding, fade transitions). PRIMARY skill for NEW deck creation of any style, incl. design-system presets; delivers a PPTD project folder plus the .pptx. Triggers: make/create/generate a PPT, presentation or slide deck, Deloitte template /德勤模板, replicate from images/PDF, infographic, poster."
---

# Definition
ppt-designer is a presentation creation and export skill built around Moonshot AI's PPTD format and browser-side PPTX writer. It defines a YAML-format intermediate DSL (`.pptd`) that abstracts OOXML and keeps each page self-contained.
...
Default delivery includes PPTX export, which needs a local toolchain. **Before generating**, verify:
...
4. **Chrome / Chromium / Edge**: needed by `agent-browser` for **image QA** (`export_images.py`) and optional `--browser` PPTX path — skip this check if the model is not vision-capable (see step4). Default PPTX export uses local WASM and does **not** require a browser. If image QA later fails with a browser-launch error, ask the user to install a Chromium-based browser.
...
2. Then determine the design direction
   ...
   **⚠️ If the user has NOT clearly indicated a design direction, you MUST ask first — do NOT silently default to self-directed design.** This is mandatory before generating. Present these options (Deloitte first):
   1. **Deloitte template (德勤模板)** — built-in, for consulting / corporate reporting (D.Nova style). Recommended for Deloitte-internal use. See [Using the built-in Deloitte template](#using-the-built-in-deloitte-template).
   2. **Design-system preset (设计系统预设)** — 30 presets grouped by scenario. Offer the CATEGORY first; once the user picks a category, list the presets in it (`reference/design_system/<category>/`):
      - Consulting 咨询 — 6 presets
      - Finance 金融 — 6 presets
      - Work report 工作汇报 — 6 presets
      - Academic 学术 — 6 presets
      - Promotion / brand 品牌推广 — 6 presets
   3. **Self-directed design (自主设计)** — you choose the visual style to fit the topic.
   4. **Upload a template / style reference (上传模板或风格参考)** — the user supplies a .pptx or a style source (images, web pages).

   Only skip the ask when the user has already specified a direction, OR explicitly delegates ("你看着办" / "自由发挥" / "free reign").

   Then map the user's choice to one of these design directions:
   - Self-directed design: the user picked self-directed in the ask above, or explicitly delegated the choice ("你看着办" / "自由发挥" / "free reign"); you need to fill in or create the design
   ...
   - Built-in template (Deloitte): the user asks for a Deloitte / D.Nova / 德勤-styled deck and does NOT upload their own pptx → use the skill's built-in Deloitte PPTD template. See [Using the built-in Deloitte template](#using-the-built-in-deloitte-template) below.
...
##### Using the built-in Deloitte template

Trigger: the user asks for a Deloitte / D.Nova / 德勤-styled deck AND has NOT uploaded their own pptx. (If they uploaded one, use the generic [Using a template](#using-a-template) path instead.)

The built-in Deloitte template lives at `pptd-template/deloitte-pptd/` — already in PPTD form, so skip pptx→pptd conversion.

1. Copy the whole template directory into your project:
   ```bash
   cp -r pptd-template/deloitte-pptd/ <project>/deloitte/
   ```
2. Read `deloitte.pptd` (theme: green `#046A38`, lime `#86BC25`, gray `#53565A`, fonts) and skim several `pages/*.page` to learn its visual vocabulary.
3. Produce the deck under the **Deloitte Preservation Contract**:

> **Strictly follow the Deloitte template.** The deliverable must look like the template — same look, the user's content. You may do ONLY three things:
>
> - **修改文本 (text)** — replace template/sample text with the user's content; write proper slide copy yourself.
> - **调整页数 (pages)** — add / remove / reorder slides by duplicating or deleting whole template pages.
> - **增减元素 (elements)** — add or remove elements to fit the content — but only using the template's existing visual vocabulary (reuse its colors, fonts, shapes, components).
>
> Do NOT introduce anything foreign: no new color schemes, no non-template fonts, no foreign images / icons / illustrations, no restyling. Adding a new element is allowed only if it matches the template's look (same colors / fonts / component style); if you need a visual the template doesn't already provide, reuse or duplicate a template element — never invent one.
>
> One line: **stay strictly inside the Deloitte template's visual language; only text, page count, and elements change.**

4. Continue with the normal step4 (validation; image QA only when vision-capable) and step5 (output).
...
2. Visual review — **gated on whether the running model supports image input (multimodal/vision)**. Decide first, then take the matching branch:

   **If the running model is NOT vision-capable (you cannot read/analyze images): SKIP the image export entirely.** Do NOT run `scripts/export_images.py` — it would cost a Chromium/agent-browser setup and image export you cannot then review. Instead, run a structural review: check each `.page` for bounds, overflow-prone long text, contrast, hierarchy, and layout density over multiple rounds, and explicitly state in your final response that image-based visual QA was skipped because the model is not vision-capable. Having done that, proceed to PPTX export — the "do not export before visual review passes" rule below applies to the vision branch only.

   **If the running model IS vision-capable (multimodal):** visual review is **required before PPTX export**. The workflow bullets below apply to this vision branch only:
   - Run `scripts/export_images.py`. It loads the deck into the **local neo-ppt editor**, chooses 导出 → 图片, downloads the images ZIP, unzips it, and stitches all pages into one overview image. Needs Chromium via `agent-browser`, but **does not** call `www.kimi.com`.

     ```bash
     python3 ~/.agents/skills/ppt-designer/scripts/export_images.py \
...
4. PPTX conversion: use `scripts/export_pptx.py`. **Default path is local patched official WASM** (`scripts/local-export/export-pptd.mjs --no-sign`): no cookie, no signature API, no browser UI. Optional `--browser` uses the **local** neo-ppt mirror, still offline — not `www.kimi.com`.
...
6. Export command:
   ...
   python3 ~/.agents/skills/ppt-designer/scripts/export_pptx.py \
     /abs/path/project/deck.pptd \
