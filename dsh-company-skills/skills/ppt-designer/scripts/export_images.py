...
def export_images(
    ...
    with temporary_directory(prefix="ppt-designer-images-") as temp_name:
        temp_dir = Path(temp_name)
        download_dir = temp_dir / "downloads"
        download_dir.mkdir()
        server, thread, url = serve_local_editor(payload)
        session = f"ppt-designer-images-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print(f"ppt-designer image export failed: {exc}", file=sys.stderr)
        return 1
