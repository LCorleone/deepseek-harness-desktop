...
Optional --browser path: local neo-ppt mirror via agent-browser
  (local neo-ppt UI, no www.kimi.com).
...
EDITOR_MISSING_HINT = (
    "local neo-ppt editor not found. Set OPEN_KIMI_PPT_EDITOR to the editor "
    "directory."
)
...
def log(message: str) -> None:
    print(f"[ppt-designer] {message}", file=sys.stderr, flush=True)
...
def run_command(
    ...
    *,
    ...
    """
    handle, sink_path = tempfile.mkstemp(prefix="ppt-designer-", suffix=".log")
    os.close(handle)
    sink = Path(sink_path)
    output = ""
    try:
...
def resolve_editor_root() -> Path:
    ...
    # monorepo / npm package: skills/ppt-designer → package root
...
def export_pptx(
    ...
    with temporary_directory(prefix="ppt-designer-export-") as temp_name:
        temp_dir = Path(temp_name)
        download_dir = temp_dir / "downloads"
        download_dir.mkdir()
        server, thread, url = serve_local_editor(payload)
        session = f"ppt-designer-export-{os.getpid()}-{uuid.uuid4().hex[:8]}"
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        ...
        print(f"ppt-designer export failed: {exc}", file=sys.stderr)
        return 1
