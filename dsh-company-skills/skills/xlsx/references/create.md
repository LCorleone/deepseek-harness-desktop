...
The template gives you a complete, valid 7-file xlsx skeleton (the bundle-safe
spelling `Content_Types.xml` is the OOXML `[Content_Types].xml` — `xlsx_pack.py`
accepts either and always zips it under the bracketed name):

```
/tmp/xlsx_work/
├── Content_Types.xml         ← MIME type registry (= OOXML [Content_Types].xml)
├── _rels/
...
**File 3 of 4 — `Content_Types.xml` (the OOXML `[Content_Types].xml`; keep the
on-disk name — `xlsx_pack.py` renames it into the zip)** (MIME type declarations):
...
`xlsx_pack.py` will:
1. Check that `[Content_Types].xml` exists at the root (the template's bracket-free `Content_Types.xml` spelling is accepted and zipped as `[Content_Types].xml`)
