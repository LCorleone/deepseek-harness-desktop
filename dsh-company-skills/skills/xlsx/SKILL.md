---
name: xlsx
description: "Open, create, read, analyze, edit, or validate Excel/spreadsheet files (.xlsx, .xlsm, .csv, .tsv): new xlsx from scratch, reading and analyzing existing files, editing with zero format loss, formula recalculation and validation, professional financial formatting standards. Triggers: 'spreadsheet', 'Excel', '.xlsx', '.csv', 'pivot table', 'financial model', 'formula', or any request to produce tabular data in Excel format."
license: MIT
---
...
Run `formula_check.py` for static validation. `libreoffice_recalc.py` (dynamic recalculation) needs LibreOffice/`soffice`, which is not bundled with this skill — three-stage rule: **① check** `where.exe soffice` / `soffice --version` (PATH and this skill's `tools/` dir); **② if missing, try install** user-level first — portable edition, or the official installer (admin may be needed for the installer); downloads go through the corporate proxy, sandbox escalation approval applies as usual; **③ only on install failure**, treat `formula_check.py` static validation as the only automated check here.
