...
import { FIXTURE_NAMES, PACKAGE_ROOT, REPO_ROOT, toolsBundle, toolsCodec, toolsReleaseSurface } from './tools.js'
...
/** Canary lines that must never appear in a packed artifact. */
const CANARIES = [
  'FIXTURE-HELLO-PLAINTEXT-CANARY',
  'FIXTURE-NOTES-PLAINTEXT-CANARY',
  'fixture-hello would read',
]
...
function packFixtures(extension: string, directory = scratch()): string {
  ...
  execFileSync(process.execPath, [PACKER, '--skills', FIXTURES_DIR, '--out', out], { cwd: REPO_ROOT_PATH })
...
function packedSkill(name = 'fixture-hello') {
  return toolsBundle.readSkillDirectory(join(FIXTURES_DIR, name))
}
...
describe('company skills container decode', () => {
  ...
  it('decodes the shipped asset into one index entry per fixture skill', () => {
    const entries = decodeContainer(readFileSync(ASSET_PATH, 'utf8'))
    expect(entries.map((entry) => entry.name)).toEqual(FIXTURE_NAMES)
    for (const entry of entries) {
      expect(entry.description).toBe(packedSkill(entry.name).description)
    }
  })
  ...
  it('reads the packed container the same way the packer canonicalized it', () => {
    ...
    for (const entry of entries) {
      ...
      expect(reconstructed.map((file) => file.path).sort()).toEqual(listFiles(join(FIXTURES_DIR, bundle.name)))
      for (const file of reconstructed) {
        expect(file.bytes.equals(readFileSync(join(FIXTURES_DIR, bundle.name, file.path)))).toBe(true)
      }
    }
  })
  ...
  it('is deterministic: the same fixtures always produce the same bytes', () => {
    expect(packFixtures('.bundle')).toBe(packFixtures('.bundle'))
    expect(packFixtures('.bundle')).toBe(readFileSync(ASSET_PATH, 'utf8'))
  })
  ...
  it('carries no plaintext of any bundled skill', () => {
    ...
    expect(artifact.includes('# Fixture hello')).toBe(false)
    ...
    const container = JSON.parse(decodeBundleBlob(extractBundleBlob(artifact))) as {
      skills: readonly { name: string; assets: readonly { path: string; content: string }[] }[]
    }
    expect(JSON.stringify(container)).toContain('# Fixture hello')
    for (const [name, canary] of [['fixture-hello', CANARIES[0]], ['fixture-notes', CANARIES[1]]] as const) {
      const skill = container.skills.find((candidate) => candidate.name === name)
      const note = skill?.assets.find((entry) => entry.path === 'assets/notes.md')
      expect(note, `${name} must carry its notes asset`).toBeDefined()
      expect(Buffer.from(note?.content ?? '', 'base64').toString('utf8')).toContain(canary)
    }
  })
...
describe('package release surface', () => {
  it('ships the container asset and no plaintext source', () => {
    ...
    for (const root of ['fixtures', 'tests', 'scripts', 'src']) {
