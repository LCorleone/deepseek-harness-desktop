...
（`tools/company-skills`），本包负责读，批③加脚本执行通道，批④收编首批真实 skill。
...
corepack yarn workspace dsh-company-skills test           # vitest，22 个用例
...
- **批③**加脚本执行通道：`ctx.tools.register` 加 `ctx.subprocess.spawn`，脚本正文走 stdin，
  于是 `scripts/…` 条目零落盘即可运行。本包已经带着脚本字节，但不执行它们。
