...
This release restores AgentTeams activity and member history after a DeepSeek Harness client or host restart. It fixes the rc.8 compatibility regressions reported in [#60](https://github.com/dsh-dai-agent-teams/issues/60) and [#61](https://github.com/dsh-dai-agent-teams/issues/61).
...
```sh
dsh plugin --profile web add dsh-dai-agent-teams
```
...
本次更新修复 DeepSeek Harness 客户端或 host 重启后 AgentTeams 活动面板与成员历史消失的问题，对应 [#60](https://github.com/dsh-dai-agent-teams/issues/60) 和 [#61](https://github.com/dsh-dai-agent-teams/issues/61) 中报告的 rc.8 兼容性回归。
