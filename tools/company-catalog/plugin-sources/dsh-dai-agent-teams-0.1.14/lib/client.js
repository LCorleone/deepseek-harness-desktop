window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		const ACTIVITY_STATE_URL = "/plugins/dsh-dai-agent-teams/state";
		const ACTIVITY_HALT_URL = "/plugins/dsh-dai-agent-teams/halt";
		/**
		...
		* uses the lead whale.
		* @module dsh-dai-agent-teams/client/artwork
		*/
		/** Artwork route prefix served by the plugin host half. */
		const ART_BASE = "/plugins/dsh-dai-agent-teams/assets/";
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/home/runner/work/dsh-dai-agent-teams/dsh-dai-agent-teams/src/client/AgentTeamsCard.module.css.mjs
		...
		* @module dsh-dai-agent-teams/client/card
		*/
		...
		* @module dsh-dai-agent-teams/client/staging-plan
		*/
		const PLAN_URL = "/plugins/dsh-dai-agent-teams/plan";
		...
		*/
		...
		const PANEL_LAYOUT_STORAGE_KEY = "dsh-dai-agent-teams:activity-panel:v1";
		...
		* polls the host `/plugins/dsh-dai-agent-teams/state` route for
		...
		* @module dsh-dai-agent-teams/client/activity
		*/
		...
		* claimed/pending/failed/cancelled keep their own labels and colors. */
		...
		/** Phase key of the current live team: planning / running / completed. */
		function badgeStatusPhase(team) {
			if (team === void 0) return "completed";
			if (team.halted === true) return "stopped";
			if (team.tasks.some((task) => task.status === "failed")) return "stopped";
			if (team.phase === "staged") return "planning";
			const working = team.members.some((member) => member.activity === "working" || member.status === "working");
			const running = team.tasks.some((task) => task.status === "claimed" || task.status === "in_progress");
			if (working || running) return "running";
			const finished = team.tasks.length > 0 && team.tasks.every((task) => task.status === "completed" || task.status === "cancelled");
			if (finished) return "completed";
			return "running";
		}
		/** Badge status label derived from the current live team. */
		function badgeStatusLabel(team, t) {
			const phase = badgeStatusPhase(team);
			if (phase === "stopped") return t("badge.status.stopped");
			if (phase === "planning") return t("badge.status.planning");
			if (phase === "running") return t("badge.status.running");
			return t("badge.status.completed");
		}
		/** Collapsed badge: an always-visible corner pill while any team exists. */
		function CollapsedBadge({ label, phase, busy, onClick, t, badgeRef, onDragStart, onDrag, onDragEnd }) {
		...
		* durable roster through the same rebuilt panel instead of a second UI. */
		...
		function ActivityPanel({ sessionsList, modelDirectories, openMember, t }) {
			...
			const badgeDragRef = (0, react.useRef)(null);
			const badgeDragSuppressRef = (0, react.useRef)(false);
			const currentTeamForBadge = visibleTeams.length > 0 ? visibleTeams[0] : void 0;
			const badgeLabel = (0, react.useMemo)(() => badgeStatusLabel(currentTeamForBadge, t), [currentTeamForBadge, t]);
			const badgePhase = (0, react.useMemo)(() => badgeStatusPhase(currentTeamForBadge), [currentTeamForBadge]);
			const beginBadgeDrag = (0, react.useCallback)((event) => {
				...
				badgeDragRef.current = {
					...
					scale: rect.width > 0 && el.offsetWidth > 0 ? rect.width / el.offsetWidth : 1,
					moved: false
				};
				badgeDragSuppressRef.current = false;
				el.style.right = "auto";
				try {
					el.setPointerCapture(event.pointerId);
				} catch (error) {
					// pointer capture is best-effort; dragging still works over the badge
				}
				event.preventDefault();
			}, []);
			const updateBadgeDrag = (0, react.useCallback)((event) => {
				const gesture = badgeDragRef.current;
				const el = badgeRef.current;
				if (gesture === null || el === null || gesture.pointerId !== event.pointerId) return;
				const dx = (event.clientX - gesture.originX) / gesture.scale;
				const dy = (event.clientY - gesture.originY) / gesture.scale;
				if (Math.hypot(dx, dy) > 4) gesture.moved = true;
				el.style.left = `${gesture.startLeft + dx}px`;
				el.style.top = `${gesture.startTop + dy}px`;
			}, []);
			const endBadgeDrag = (0, react.useCallback)((event) => {
				const gesture = badgeDragRef.current;
				const el = badgeRef.current;
				if (gesture === null || gesture.pointerId !== event.pointerId) return;
				if (gesture.moved) badgeDragSuppressRef.current = true;
			...
			return (0, react_jsx_runtime.jsxs)(react_jsx_runtime.Fragment, { children: [!expanded && (0, react_jsx_runtime.jsx)(CollapsedBadge, {
				...
				t,
				badgeRef,
				onDragStart: beginBadgeDrag,
				onDrag: updateBadgeDrag,
				onDragEnd: endBadgeDrag,
				onClick: () => {
					if (badgeDragSuppressRef.current) {
						badgeDragSuppressRef.current = false;
						return;
					}
					if (current === void 0) return;
		...
		*/
		...
		const zh = {
			...
			"badge.status.planning": "Agent Teams任务规划中...",
			"badge.status.running": "Agent Teams任务运行中...",
			"badge.status.completed": "Agent Teams任务已完成",
			"badge.status.stopped": "Agent Teams任务已停止",
