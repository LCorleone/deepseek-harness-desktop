 ...
 * results.
 *
 * Installation (bundle): `dsh plugin --profile <name> add dsh-dai-agent-teams`
...
export function apply(ctx, config) {
    ...
    const registerWebSurface = () => {
        ...
        ctx.effect(() => webServer.register({
            kind: 'exact',
            path: '/plugins/dsh-dai-agent-teams/state',
        ...
        ctx.effect(() => webServer.register({
            kind: 'exact',
            path: '/plugins/dsh-dai-agent-teams/halt',
            handler: async (req, res) => {
                if (req.method !== 'POST') {
        ...
        ctx.effect(() => webServer.register({
            kind: 'exact',
            path: '/plugins/dsh-dai-agent-teams/plan',
            handler: async (req, res) => {
                if (req.method !== 'POST') {
                    res.writeHead(405, { allow: 'POST', 'cache-control': 'no-store' });
                    res.end();
        ...
        ctx.effect(() => webServer.register({
            kind: 'prefix',
            path: '/plugins/dsh-dai-agent-teams/assets',
            handler: async (req, res) => {
                let name;
                try {
