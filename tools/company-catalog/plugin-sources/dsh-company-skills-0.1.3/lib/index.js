...
*/
function decodeV2Payload(payload, what) {
	const headerEnd = payload.indexOf(58, V2_MAGIC_BYTES.length);
	...
	if (codec !== "br") throw new Error(`${what} carries wire-format v2 with the unknown compression codec "${codec}" (this decoder understands br = brotli)`);
...
*/
const CHILD_ENV_NAME_PATTERN = /^[A-Z][A-Z0-9_]*$/u;
...
function resolveLimits(options) {
	const deadlineBySkill = {};
	for (const [skill, deadline] of Object.entries(options.deadlineBySkill ?? {})) {
		if (!SKILL_NAME_PATTERN.test(skill)) throw new Error(`dsh-company-skills: deadlineBySkill key ${JSON.stringify(skill)} must be a kebab-case skill name`);
		assertPositiveInteger(`deadlineBySkill[${JSON.stringify(skill)}]`, deadline);
		if (deadline > MAX_TIMER_DELAY_MS) throw new Error(`dsh-company-skills: deadlineBySkill[${JSON.stringify(skill)}] must be no greater than ${String(MAX_TIMER_DELAY_MS)}`);
	...
	const limits = {
		...
		maxListEntries: options.maxListEntries ?? 1e3,
		deadlineBySkill
	};
...
*/
function resolveChildEnv(fragment) {
	if (fragment === void 0) return {};
	const resolved = {};
	for (const [name, value] of Object.entries(fragment)) {
		if (!CHILD_ENV_NAME_PATTERN.test(name)) throw new Error(`dsh-company-skills: childEnv name ${JSON.stringify(name)} must be an uppercase environment name (letters, digits, underscores)`);
		if (typeof value !== "string" || value.length === 0) throw new Error(`dsh-company-skills: childEnv ${JSON.stringify(name)} must carry a non-empty string value`);
		resolved[name] = value;
	}
...
* probing.
*/
function normalizeListPrefix(path) {
...
function createScriptExecutor(options) {
	...
	async function run(request) {
		...
		const failLaunch = (command, error) => {
			if (timedOut) throw new SkillRunError(`company skill "${bundle.name}" script "${script.path}" timed out after ${String(timeoutMs)} ms`);
	...
	*/
	async function list(request) {
		const loaded = options.catalog.skill(request.skill);
		if (!loaded.ok) throw new SkillRunError(`cannot list company skill "${request.skill}": ${loaded.reason}`);
		const bundle = loaded.bundle;
		const prefix = normalizeListPrefix(request.path);
		const matches = [...bundle.scripts, ...bundle.assets].map((entry) => entry.path).filter((path) => prefix === "" || path === prefix || path.startsWith(`${prefix}/`)).sort(compareCodePoints);
...
*/
const COMPANY_SKILLS_EXECUTION_ENVIRONMENT_SLOT = Symbol.for("dsh.companySkillsExecutionEnvironment");
/** globalThis narrowed to the slot; the slot is only touched through the helpers below. */
const executionEnvironmentGlobals = globalThis;
...
*/
function companySkillsExecutionEnvironment() {
	const fragment = executionEnvironmentGlobals[COMPANY_SKILLS_EXECUTION_ENVIRONMENT_SLOT];
	if (fragment === void 0) return void 0;
	if (typeof fragment !== "object" || Array.isArray(fragment)) return void 0;
	const validated = {};
	for (const [name, value] of Object.entries(fragment)) {
		if (!ENV_NAME_PATTERN.test(name)) return void 0;
...
const COMPANY_SKILL_LIST_TOOL_NAME = "company_skill_list";
/**
...
function renderListValue(value) {
	...
	if (value.total === 0) return `company skill "${value.skill}" carries no entries${scope}`;
	const noun = value.total === 1 ? "entry" : "entries";
	const shown = value.truncated ? ` (showing the first ${String(value.entries.length)})` : "";
	const lines = [`company skill "${value.skill}" carries ${String(value.total)} ${noun}${scope}${shown}:`, ...value.entries];
	if (value.truncated) lines.push(`… truncated, ${String(value.total - value.entries.length)} more — narrow the path prefix`);
	return lines.join("\n");
}
/**
...
* @returns a registry-ready tool definition.
*/
function createCompanySkillListTool(executor) {
	return defineTool({
		name: COMPANY_SKILL_LIST_TOOL_NAME,
		description: `List the entry paths one company skill carries inside its bundle — every script under \`scripts/\` and every resource at its own bundle-relative path — as a sorted list of names, never content. Company-skill bundles are opaque to the workspace, so \`bash\` or \`read\` cannot enumerate them and entry names must match the skill's own names exactly (guessing fails: the design presets live under names like \`reference/design_system/finance/black-gold-ledger/design.md\`). Use company_skill_list to discover the exact paths a company skill carries, then company_skill_read to load one (and company_skill_run to execute a \`scripts/…\` entry). \`path\` optionally narrows the listing to a bundle-root-relative directory prefix such as "reference/design_system/finance" (an exact entry path matches itself); omit it to list every entry. A prefix that matches nothing returns a normal empty listing, not an error — discovery is probing. Listings longer than ${String(executor.limits.maxListEntries)} entries keep the head and end with an explicit truncation marker; narrow the path prefix to page through the rest.`,
		parameters: {
			skill: {
				type: "string",
				required: true,
		...
		output: {
			...
			render: (_args, value) => [{
				type: "text",
				text: renderListValue(value)
			}]
		},
		async execute(args) {
			return toListValue(await executor.list({
				skill: args.skill,
		...
		presentCall: (args) => ({
			card: "generic",
			kind: "search",
			title: `List ${args.skill}${args.path === void 0 ? "" : `/${args.path}`}`,
...
*/
function maximumDeadlineMs(limits) {
...
*/
const SKILL_DEADLINE_OVERRIDES_MS = Object.freeze({
	ocr: 12e5,
	"vlm-image": 12e5,
	"smart-pdf-parser": 36e5
});
...
//#endregion
export { SKILLS_BUNDLE_URL, SKILL_DEADLINE_OVERRIDES_MS, apply, catalog, inject, name };
