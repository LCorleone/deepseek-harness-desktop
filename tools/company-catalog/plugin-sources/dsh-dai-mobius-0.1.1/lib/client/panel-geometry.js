...
export const PANEL_LAYOUT_STORAGE_KEY = 'dsh-mobius:activity-panel:v1';
export const PANEL_BADGE_OFFSET_STORAGE_KEY = 'dsh-mobius:activity-badge-offset:v1';
...
export function parseBadgeOffset(value) {
    if (value === null)
        return { x: 0, y: 0 };
    try {
        const parsed = JSON.parse(value);
        ...
        if (!finite(record.x) || !finite(record.y))
            return { x: 0, y: 0 };
        return {
            x: clamp(record.x, -BADGE_OFFSET_LIMIT, BADGE_OFFSET_LIMIT),
            y: clamp(record.y, -BADGE_OFFSET_LIMIT, BADGE_OFFSET_LIMIT),
        };
    }
    catch {
        return { x: 0, y: 0 };
