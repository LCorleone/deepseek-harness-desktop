...
async function spawnStepAgent(ctx, config, selections, captain, workflow, step, llmSelection, assignmentPrompt) {
    ...
    // A derived (Eureka-spawned) node must not discover further: one node may
    // build new nodes, but those new nodes never build again — no nested
    // expansion that would turn a few concurrent stages into a huge tree.
    if (step.derived === true)
        deny.push('mobius_discover');
    if (workflow.disableWebSearch === true) {
                ...
                if (adds.length > 0) {
                    ...
                    for (const add of adds) {
                        ...
                        fresh.steps.push({
                            ...
                            status: 'pending',
                            // Derived node: its sub-agent must not discover further, so the
                            // pipeline stays one-level deep (no nested expansion).
                            derived: true,
                            createdAt: now,
                            updatedAt: now,
                        });
