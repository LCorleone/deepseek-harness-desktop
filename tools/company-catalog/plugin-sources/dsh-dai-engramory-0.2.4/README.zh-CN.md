...
```sh
dsh plugin --profile <名字> add dsh-dai-engramory
```
...
| `registerSkill` | `true` | 设为 `false` 则保留上限、但跳过运行时召回 skill。 |
| `registerCommand` | `true` | 设为 `false` 则跳过 `/engramory` 召回命令。 |
| `skill` | 内置 | 用你自己的 markdown 替换 skill 正文。 |
| `memoryRoot` | `~/.dsh/.engramory-memory` | 记忆库根目录,供 Web 设置页与 `/engramory` 命令定位索引与召回模式文件。 |
| `mode` | 未设置 | 硬锁定召回模式(见下)。留空以便选择存在设置页里并持久化在库内。 |

## 召回模式

Engramory 可以配置为自动、按需或从不召回记忆。选择在 Web **设置 → Engramory 记忆** 里进行
(以库内一个 `mode` 文件保存),重启与读取同一文件夹的其它宿主都会沿用同一选择。

| 模式 | 常驻召回 skill | `/engramory` 命令 | 读取记忆库 |
|---|---|---|---|
| `global`(默认) | ✔ 注册 —— 每个任务开始时自动召回记忆 | ✔ 按需召回 | 放行 |
| `explicit` | ✘ —— 没有任何东西催促自动召回 | ✔ 通过 `/engramory` 按需召回 | 放行 |
| `off` | ✘ | ✘ —— 完全没有召回路径 | **拦截** —— `read`/`view`/`glob`/`grep` 读取库内任何文件都会被拒绝 |

在 `off` 模式下,插件的守卫会拒绝任何**读取或列举**记忆库(索引或笔记文件)的尝试,因此即便
有常驻指令提示指向记忆库,模型也无法真正召回记忆。写入仍走正常的容量规则。

## Web 设置页设置栏

在带图形界面的 dsh(Web)上,插件往 **设置** 里加了一个 **"Engramory 记忆"** 设置栏:
一个三选一的选择器(`global` / `explicit` / `off`),实时切换召回模式。它读取
`GET /engramory/status`,写入 `POST /engramory/mode`。主机不带 Web 时(纯 CLI/headless)该设置栏
自动缺席;上限、skill 与命令仍按配置/持久化的模式生效。
