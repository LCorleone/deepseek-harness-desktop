...
```sh
dsh plugin --profile <name> add dsh-dai-engramory
```
...
| `registerSkill` | `true` | Set `false` to keep the cap but skip the runtime recall skill entirely. |
| `registerCommand` | `true` | Set `false` to skip the `/engramory` recall command entirely. |
| `skill` | built-in | Replace the skill body with your own markdown. |
| `memoryRoot` | `~/.dsh/.engramory-memory` | Memory-store root the Web settings panel and the `/engramory` command use to locate the index and the recall-mode file. |
| `mode` | unset | Hard-lock the recall mode (see below). Leave unset so the choice lives in Settings and persists in the store. |

## Recall mode

Engramory can be configured to recall memory automatically, on demand, or never. The choice
is made in the Web **Settings → Engramory 记忆** section (a `mode` file in the store), and
survives restarts and other hosts reading the same folder.

| Mode | Always-on recall skill | `/engramory` command | Reading the store |
|---|---|---|---|
| `global` (default) | ✔ registered — memory is recalled at the start of every task | ✔ on-demand recall | allowed |
| `explicit` | ✘ — nothing nudges auto-recall | ✔ on-demand recall via `/engramory` | allowed |
| `off` | ✘ | ✘ — no recall path at all | **blocked** — `read`/`view`/`glob`/`grep` of any file in the store is refused |

In `off` mode the plugin's guard refuses any attempt to **read or list** the memory store
(the index or the note files), so the model has no way to recall memory even if a standing
instruction nudge points at it. Writes still go through the normal cap rule.

## Web settings section

On a graphical dsh (Web) host the plugin adds an **"Engramory 记忆"** section to **Settings**:
a three-choice selector (`global` / `explicit` / `off`) that switches the recall mode live.
It reads `GET /engramory/status` and writes `POST /engramory/mode`. On a host without Web
(pure CLI/headless) the section is simply absent; the cap, skill, and command follow the
configured/persisted mode regardless.
...
| `read`, `view`, or any unknown tool (global/explicit) | allow — recall must never be blocked, even over cap |
| `read`/`view`/`glob`/`grep` of a file in the store (`off`) | **deny** — memory is disabled, nothing can be recalled |
