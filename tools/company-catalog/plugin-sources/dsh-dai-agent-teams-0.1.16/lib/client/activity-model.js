 ...
 */
export function panelPhaseOf(live, archived = []) {
    const livePhase = phaseOfTeams(live);
    if (livePhase !== null)
        return livePhase;
    const archivedPhase = phaseOfTeams(archived);
    return archivedPhase ?? 'idle';
}
function phaseOfTeams(teams) {
    if (teams.length === 0)
        return null;
    const anyStaged = teams.some((team) => team.phase === 'staged');
    if (anyStaged)
        return 'planning';
    const anyHalted = teams.some((team) => team.halted === true);
    if (anyHalted)
        return 'halted';
    const anyFailed = teams.some((team) => (team.tasks ?? []).some((task) => task.status === 'failed'));
    if (anyFailed)
        return 'failed';
    const allSettled = teams.every((team) => {
        const tasks = team.tasks ?? [];
        return tasks.length > 0 && tasks.every((task) => (task.status === 'completed' || task.status === 'failed' || task.status === 'cancelled'));
    });
    return allSettled ? 'completed' : 'running';
}
/** Whether the floater shows its animated water-wave surface (any real state). */
export function panelRunsWater(phase) {
    return phase !== 'idle';
}
