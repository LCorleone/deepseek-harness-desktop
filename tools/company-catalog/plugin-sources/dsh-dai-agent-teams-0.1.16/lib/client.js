window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		*/
		...
		/**
		* Derive the panel's overall phase from the teams owned by the current
		* captain session, live teams taking priority over already-archived ones.
		*
		* - `idle`: no team in this session (the collapsed badge / empty state).
		* - `planning`: any live team is still in the staged review phase (before
		*   Approve & Run), so the floater reads "AgentTeams 规划中".
		* - `halted`: any team was manually stopped by its captain (`halted`), so
		*   the floater reads "AgentTeams 已中断".
		* - `failed`: any team has at least one failed task, so the floater reads
		*   "AgentTeams 失败".
		* - `running`: any team is approved and working (or waiting on scheduled
		*   work) but not yet fully delivered — "AgentTeams 运行中".
		* - `completed`: every team in scope is fully settled (all tasks are in a
		*   terminal state) — "AgentTeams 已完成".
		*
		* A completed team is archived (moved out of live), so a session whose live
		* set is empty but that owns archived teams must still report the archived
		* verdict (e.g. "已完成") instead of degrading to the running fallback.
		* Live teams always win: ongoing work shadows archived history.
		*
		* Priority: a manual stop dominates the verdict, so a halted team never reads
		* as merely running or completed.
		*/
		function panelPhaseOf(live, archived = []) {
			const livePhase = phaseOfTeams(live);
			if (livePhase !== null) return livePhase;
			return phaseOfTeams(archived) ?? "idle";
		}
		function phaseOfTeams(teams) {
			if (teams.length === 0) return null;
			if (teams.some((team) => team.phase === "staged")) return "planning";
			if (teams.some((team) => team.halted === true)) return "halted";
			if (teams.some((team) => (team.tasks ?? []).some((task) => task.status === "failed"))) return "failed";
			return teams.every((team) => {
				const tasks = team.tasks ?? [];
				return tasks.length > 0 && tasks.every((task) => task.status === "completed" || task.status === "failed" || task.status === "cancelled");
			}) ? "completed" : "running";
		}
		/** Whether the floater shows its animated water-wave surface (any real state). */
		function panelRunsWater(phase) {
			return phase !== "idle";
		}
		...
		*/
		...
		//#endregion
		//#region \0dsh-css:/opt/dsh-plugins/dsh-agent-teams/src/client/AgentTeamsCard.module.css.mjs
		const css$1 = ".a3mCCW_root{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:10px;flex-direction:column;gap:8px;width:100%;min-width:0;padding:10px 12px;display:flex}.a3mCCW_head{align-items:center;gap:8px;min-width:0;display:flex}.a3mCCW_leadAvatar{object-fit:contain;filter:drop-shadow(0 1px 1px #122d4833);background:0 0;border:0;border-radius:0;flex:none;width:30px;height:30px}.a3mCCW_teamName{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:0 auto;font-size:13px;font-weight:600;line-height:20px;overflow:hidden}.a3mCCW_memberCount{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;margin-left:auto;font-size:11px;line-height:16px}.a3mCCW_panelButton{border:1px solid var(--dsw-alias-line-strong);background:var(--dsw-alias-bg-module);color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;flex:none;padding:2px 8px;font-size:10.5px;font-weight:600;line-height:16px;transition:border-color .12s,color .12s}.a3mCCW_panelButton:hover{border-color:var(--dsw-alias-state-business-primary);color:var(--dsw-alias-state-business-primary)}.a3mCCW_panelButton:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.a3mCCW_members{flex-wrap:wrap;gap:6px;min-width:0;display:flex}.a3mCCW_member{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);max-width:160px;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border-radius:999px;align-items:center;gap:5px;padding:3px 8px 3px 3px;font-size:11px;font-weight:500;line-height:16px;transition:border-color .12s,background-color .12s;display:inline-flex}.a3mCCW_member:hover{border-color:var(--dsw-alias-state-business-primary);background:var(--dsw-alias-bg-fill-neutral)}.a3mCCW_member:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:1px}.a3mCCW_memberArt{object-fit:contain;filter:drop-shadow(0 1px 1px #122d482e);background:0 0;border:0;border-radius:0;width:24px;height:24px}.a3mCCW_memberInitial{background:var(--dsw-alias-bg-fill-business);width:20px;height:20px;color:var(--dsw-alias-label-on-fill);border-radius:50%;justify-content:center;align-items:center;font-size:10px;font-weight:600;line-height:20px;display:inline-flex}.a3mCCW_memberName{text-overflow:ellipsis;white-space:nowrap;min-width:0;overflow:hidden}";
		const tagId$1 = "dsh-dai-agent-teams/AgentTeamsCard.module.css";
		if (typeof document !== "undefined" && document.querySelector("style[data-plugin-css=" + JSON.stringify(tagId$1) + "]") === null) {
			const tag = document.createElement("style");
			tag.dataset.plugin = "dsh-dai-agent-teams";
		...
		var AgentTeamsCard_module_css_default = {
			"head": "a3mCCW_head",
			"leadAvatar": "a3mCCW_leadAvatar",
			"member": "a3mCCW_member",
			"memberArt": "a3mCCW_memberArt",
			"memberCount": "a3mCCW_memberCount",
			"memberInitial": "a3mCCW_memberInitial",
			"memberName": "a3mCCW_memberName",
			"members": "a3mCCW_members",
			"panelButton": "a3mCCW_panelButton",
			"root": "a3mCCW_root",
			"teamName": "a3mCCW_teamName"
		};
		//#endregion
		...
		* (historical session review). */
		...
		//#endregion
		//#region \0dsh-css:/opt/dsh-plugins/dsh-agent-teams/src/client/ActivityPanel.module.css.mjs
		const css = "html{--agent-teams-panel-shift:420px}html[data-agent-teams-panel-open] [data-phase=active]{box-sizing:border-box;padding-right:var(--agent-teams-panel-shift)}.q4VXeG_badge,.q4VXeG_panel{--dsw-alias-line-normal:var(--dsw-static-neutral-bluish-150,#e7e9ee);--dsw-alias-line-strong:color-mix(in srgb, var(--dsw-static-neutral-bluish-200,#e1e5ee) 50%, var(--dsw-static-neutral-bluish-300,#cfd3d6));--dsw-alias-bg-module:var(--dsw-alias-bg-layer-1,#fff);--dsw-alias-bg-fill-neutral:var(--dsw-static-neutral-bluish-100,#eef0f4);--dsw-alias-bg-fill-business:var(--dsw-alias-state-business-primary,#4d6bfe);--dsw-alias-bg-fill-success:var(--dsw-alias-state-success-primary,#12a150);--dsw-alias-bg-fill-warning:var(--dsw-alias-state-warn-primary,#e08700);--dsw-alias-bg-fill-danger:var(--dsw-alias-state-error-primary,#e5484d);--dsw-alias-state-success:var(--dsw-alias-state-success-primary,#12a150);--dsw-alias-state-warning:var(--dsw-alias-state-warn-primary,#e08700);--dsw-alias-state-danger:var(--dsw-alias-state-error-primary,#e5484d);--dsw-alias-label-on-fill:var(--dsw-alias-label-primary-inverted,#fff)}.q4VXeG_badge{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 92%, transparent);backdrop-filter:blur(16px);height:34px;box-shadow:0 8px 28px color-mix(in srgb, var(--dsw-alias-label-primary) 14%, transparent);color:var(--dsw-alias-label-secondary);font:inherit;cursor:grab;touch-action:none;border-radius:999px;align-items:center;gap:7px;padding:0 12px;font-size:12px;font-weight:600;line-height:20px;transition:border-color .15s;display:inline-flex;position:absolute;top:64px;right:18px;overflow:hidden}.q4VXeG_badge[data-phase=planning]{--agent-teams-accent:#4d6bfe}.q4VXeG_badge[data-phase=running]{--agent-teams-accent:#f2994a}.q4VXeG_badge[data-phase=completed]{--agent-teams-accent:#12a150}.q4VXeG_badge[data-phase=failed]{--agent-teams-accent:#e5484d}.q4VXeG_badge[data-phase=halted]{--agent-teams-accent:#8a8f98}.q4VXeG_badge:before,.q4VXeG_badge:after{z-index:0;content:\"\";pointer-events:none;border-radius:inherit;position:absolute;inset:-40%}.q4VXeG_badge:before{background:radial-gradient(ellipse at 20% 30%, color-mix(in srgb, var(--agent-teams-accent) 30%, transparent) 0%, transparent 48%);animation:4.5s ease-in-out infinite q4VXeG_agentTeamsBadgeWave}.q4VXeG_badge:after{background:radial-gradient(ellipse at 70% 20%, color-mix(in srgb, var(--agent-teams-accent) 20%, transparent) 0%, transparent 52%);animation:6.5s ease-in-out infinite reverse q4VXeG_agentTeamsBadgeWave}@keyframes q4VXeG_agentTeamsBadgeWave{0%{opacity:.45;transform:translate(-18%)rotate(0)}50%{opacity:.9;transform:translate(18%,2%)rotate(2deg)}to{opacity:.45;transform:translate(-18%)rotate(0)}}.q4VXeG_badge>.q4VXeG_badgeDot,.q4VXeG_badge>.q4VXeG_badgeText,.q4VXeG_badge>.q4VXeG_badgeCount{z-index:1;position:relative}.q4VXeG_badge:hover{border-color:var(--dsw-alias-line-strong)}.q4VXeG_badge:active,.q4VXeG_badge[data-dragging=true]{cursor:grabbing;user-select:none}.q4VXeG_badge[data-dragging=true]{box-shadow:0 8px 28px color-mix(in srgb, var(--dsw-alias-label-primary) 14%, transparent), 0 16px 40px color-mix(in srgb, var(--dsw-alias-label-primary) 22%, transparent)}.q4VXeG_badge:focus-visible,.q4VXeG_iconButton:focus-visible,.q4VXeG_memberRow:focus-visible,.q4VXeG_membersToggle:focus-visible,.q4VXeG_sectionToggleTitle:focus-visible,.q4VXeG_dagNode:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.q4VXeG_badgeDot,.q4VXeG_panelDot{background:var(--dsw-alias-label-tertiary);border-radius:50%;width:7px;height:7px}.q4VXeG_badgeDot[data-busy=true],.q4VXeG_panelDot[data-busy=true]{background:var(--dsw-alias-state-business-primary);animation:1.25s ease-in-out infinite q4VXeG_agentTeamsPulse}.q4VXeG_badgeDot[data-phase=planning],.q4VXeG_badgeDot[data-phase=running],.q4VXeG_badgeDot[data-phase=completed],.q4VXeG_badgeDot[data-phase=failed],.q4VXeG_badgeDot[data-phase=halted]{background:var(--agent-teams-accent)}.q4VXeG_badgeText{text-overflow:ellipsis;white-space:nowrap;max-width:160px;overflow:hidden}.q4VXeG_badgeCount,.q4VXeG_memberCount,.q4VXeG_teamStats,.q4VXeG_stageLabel,.q4VXeG_taskId{font-variant-numeric:tabular-nums}.q4VXeG_panel{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--dsw-alias-line-strong) 58%, transparent);background:color-mix(in srgb, var(--dsw-alias-bg-module) 95%, transparent);backdrop-filter:blur(20px)saturate(1.08);box-shadow:0 12px 32px color-mix(in srgb, var(--dsw-alias-label-primary) 12%, transparent), 0 32px 72px color-mix(in srgb, var(--dsw-alias-label-primary) 16%, transparent);will-change:transform;border-radius:16px;flex-direction:column;animation:.16s ease-out q4VXeG_agentTeamsPanelIn;display:flex;position:absolute;top:0;left:0;overflow:hidden}.q4VXeG_panel[data-phase=planning]{--agent-teams-accent:#4d6bfe}.q4VXeG_panel[data-phase=running]{--agent-teams-accent:#f2994a}.q4VXeG_panel[data-phase=completed]{--agent-teams-accent:#12a150}.q4VXeG_panel[data-phase=failed]{--agent-teams-accent:#e5484d}.q4VXeG_panel[data-phase=halted]{--agent-teams-accent:#8a8f98}.q4VXeG_panel[data-dragging],.q4VXeG_panel[data-resizing]{user-select:none;box-shadow:0 16px 38px color-mix(in srgb, var(--dsw-alias-label-primary) 14%, transparent), 0 36px 78px color-mix(in srgb, var(--dsw-alias-label-primary) 18%, transparent)}@keyframes q4VXeG_agentTeamsPanelIn{0%{opacity:0}to{opacity:1}}@keyframes q4VXeG_agentTeamsPulse{0%,to{opacity:.42}50%{opacity:1}}.q4VXeG_waterWaves{z-index:0;border-radius:inherit;pointer-events:none;opacity:0;transition:opacity .32s;position:absolute;inset:0;overflow:hidden}.q4VXeG_waterWaves[data-active=true]{opacity:1}.q4VXeG_waterWaves:before,.q4VXeG_waterWaves:after{content:\"\";pointer-events:none;width:200%;height:260%;position:absolute;left:-50%}.q4VXeG_waterWaves:before{background:radial-gradient(ellipse at 20% 30%, color-mix(in srgb, var(--agent-teams-accent,var(--dsw-alias-state-business-primary)) 20%, transparent) 0%, transparent 42%);animation:9s ease-in-out infinite q4VXeG_agentTeamsWave;top:-70%}.q4VXeG_waterWaves:after{background:radial-gradient(ellipse at 70% 18%, color-mix(in srgb, var(--agent-teams-accent,var(--dsw-alias-state-success)) 14%, transparent) 0%, transparent 46%);animation:13s ease-in-out infinite reverse q4VXeG_agentTeamsWave;top:-52%}@keyframes q4VXeG_agentTeamsWave{0%{transform:translate(0,0)rotate(0)scale(1.02)}25%{transform:translate(6%,2%)rotate(1.2deg)scale(1.05)}50%{transform:translateY(4%)rotate(-.8deg)scale(1.03)}75%{transform:translate(-5%,1%)rotate(1deg)scale(1.06)}to{transform:translate(0,0)rotate(0)scale(1.02)}}.q4VXeG_panel>.q4VXeG_panelHead,.q4VXeG_panel>.q4VXeG_teams{z-index:1;position:relative}.q4VXeG_panelHead{border-bottom:1px solid var(--dsw-alias-line-normal);cursor:grab;touch-action:none;flex:none;justify-content:space-between;align-items:center;min-height:44px;padding:0 14px 0 16px;display:flex}.q4VXeG_panelHead:active,.q4VXeG_panel[data-dragging] .q4VXeG_panelHead{cursor:grabbing}.q4VXeG_panel[data-compact] .q4VXeG_panelHead{cursor:default;touch-action:auto}.q4VXeG_panelTitle{color:var(--dsw-alias-label-primary);align-items:center;gap:8px;font-size:14px;font-weight:600;line-height:20px;display:inline-flex}.q4VXeG_panelControls{flex:none;align-items:center;gap:2px;display:inline-flex}.q4VXeG_iconButton{width:28px;height:28px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border:0;border-radius:7px;justify-content:center;align-items:center;padding:0;transition:background-color .12s,color .12s,transform .12s;display:inline-flex}.q4VXeG_iconButton:hover{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}.q4VXeG_iconButton:active{transform:scale(.94)}.q4VXeG_iconButton[data-control=dock][data-mode=docked] svg{transform:scaleX(-1)}.q4VXeG_resizeHandle{z-index:5;touch-action:none;pointer-events:auto;position:absolute}.q4VXeG_resizeHandle[data-resize-edge=left]{cursor:ew-resize;width:14px;top:44px;bottom:14px;left:0}.q4VXeG_resizeHandle[data-resize-edge=left]:after{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 42%, transparent);content:\"\";opacity:.45;border-radius:999px;width:3px;height:48px;transition:background-color .12s,opacity .12s;position:absolute;top:50%;left:4px;transform:translateY(-50%)}.q4VXeG_resizeHandle[data-resize-edge=bottom]{cursor:ns-resize;height:14px;bottom:0;left:20px;right:20px}.q4VXeG_resizeHandle[data-resize-edge=bottom]:after{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 42%, transparent);content:\"\";opacity:.45;border-radius:999px;width:48px;height:3px;transition:background-color .12s,opacity .12s;position:absolute;bottom:4px;left:50%;transform:translate(-50%)}.q4VXeG_resizeHandle[data-resize-edge=corner]{cursor:nwse-resize;width:28px;height:28px;bottom:0;right:0}.q4VXeG_resizeHandle[data-resize-edge=corner]:after{border-right:2px solid var(--dsw-alias-label-tertiary);border-bottom:2px solid var(--dsw-alias-label-tertiary);content:\"\";opacity:.58;width:10px;height:10px;transition:border-color .12s,opacity .12s;position:absolute;bottom:6px;right:6px}.q4VXeG_resizeHandle:hover:after,.q4VXeG_panel[data-resizing] .q4VXeG_resizeHandle:after{background-color:var(--dsw-alias-bg-fill-business);border-color:var(--dsw-alias-bg-fill-business);opacity:.95}.q4VXeG_teams{overscroll-behavior:contain;scrollbar-color:color-mix(in srgb, var(--dsw-alias-label-tertiary) 28%, transparent) transparent;scrollbar-width:thin;flex-direction:column;min-height:0;display:flex;overflow-y:auto}.q4VXeG_teams::-webkit-scrollbar{width:6px}.q4VXeG_teams::-webkit-scrollbar-track{background:0 0}.q4VXeG_teams::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 28%, transparent);background-clip:padding-box;border:2px solid #0000;border-radius:999px}.q4VXeG_teams:hover::-webkit-scrollbar-thumb{background:color-mix(in srgb, var(--dsw-alias-label-tertiary) 44%, transparent);background-clip:padding-box}.q4VXeG_team{border-bottom:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:12px;padding:12px 14px 16px;display:flex;container:q4VXeG_agent-team/inline-size}.q4VXeG_team:last-child{border-bottom:0}.q4VXeG_teamHead{align-items:center;gap:10px;min-width:0;display:flex}.q4VXeG_teamName{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;flex:1;font-size:13px;font-weight:600;line-height:18px;overflow:hidden}.q4VXeG_teamStats{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;gap:8px;font-size:10.5px;line-height:16px;display:inline-flex}.q4VXeG_teamStopButton{border:1px solid var(--dsw-alias-line-normal);width:26px;height:26px;color:var(--dsw-alias-label-tertiary);cursor:pointer;background:0 0;border-radius:7px;flex:none;place-items:center;padding:0;transition:border-color .15s,background .15s,color .15s;display:grid}.q4VXeG_teamStopButton:hover{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 42%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-danger) 7%, transparent);color:var(--dsw-alias-state-danger)}.q4VXeG_teamStopButton:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:2px}.q4VXeG_stopModalActions{justify-content:flex-end;gap:8px;display:flex}.q4VXeG_stopModalActions button{border:1px solid var(--dsw-alias-line-normal,#e7e9ee);background:var(--dsw-alias-bg-fill-neutral,#eef0f4);min-height:34px;color:var(--dsw-alias-label-primary,#1c1c1e);cursor:pointer;font:inherit;border-radius:8px;justify-content:center;align-items:center;gap:6px;padding:6px 13px;font-size:12px;font-weight:600;display:inline-flex}.q4VXeG_stopModalActions button[data-danger]{border-color:var(--dsw-alias-state-danger,#e5484d);background:var(--dsw-alias-state-danger,#e5484d);color:var(--dsw-alias-label-on-fill,#fff)}.q4VXeG_stopModalActions button:disabled{cursor:wait;opacity:.58}.q4VXeG_stopModalError{background:color-mix(in srgb, var(--dsw-alias-state-danger,#e5484d) 8%, transparent);color:var(--dsw-alias-state-danger,#e5484d);border-radius:8px;align-items:flex-start;gap:7px;margin:0;padding:9px 10px;font-size:12px;line-height:18px;display:flex}.q4VXeG_stopModalError svg{flex:none;margin-top:1px}.q4VXeG_sectionHead{justify-content:space-between;align-items:center;gap:8px;min-width:0;display:flex}.q4VXeG_sectionTitle{color:var(--dsw-alias-label-secondary);align-items:center;gap:6px;font-size:11px;font-weight:600;line-height:16px;display:inline-flex}.q4VXeG_sectionHint{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}.q4VXeG_delegationSection{min-width:0}.q4VXeG_captainNode{box-sizing:border-box;border:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 32%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 7%, var(--dsw-alias-bg-module));border-radius:10px;grid-template-columns:48px minmax(0,1fr) auto;align-items:center;gap:9px;min-height:56px;padding:6px 10px;display:grid}.q4VXeG_captainAvatar,.q4VXeG_memberAvatar{flex:none;justify-content:center;align-items:center;display:inline-flex;position:relative}.q4VXeG_captainAvatar{width:46px;height:46px}.q4VXeG_leadAvatar,.q4VXeG_memberArt{object-fit:contain;filter:drop-shadow(0 1px 1px #122d4833);background:0 0;border:0;border-radius:0}.q4VXeG_leadAvatar{width:44px;height:44px}.q4VXeG_memberArt{width:40px;height:40px}.q4VXeG_captainInfo,.q4VXeG_memberInfo{flex-direction:column;min-width:0;display:flex}.q4VXeG_captainInfo{gap:2px}.q4VXeG_captainLine,.q4VXeG_memberLine{align-items:center;gap:6px;min-width:0;display:flex}.q4VXeG_captainName,.q4VXeG_memberName{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:12.5px;font-weight:600;line-height:18px;overflow:hidden}.q4VXeG_captainRole,.q4VXeG_memberRole{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}.q4VXeG_captainSummary,.q4VXeG_memberStatusLine{color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;font-size:10.5px;line-height:15px;overflow:hidden}.q4VXeG_taskDetailModel{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9.5px;line-height:14px;overflow:hidden}.q4VXeG_memberModel{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-fill-neutral);min-width:0;max-width:132px;color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;border-radius:999px;flex:0 auto;align-items:center;padding:0 6px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;font-weight:600;line-height:15px;display:inline-flex;overflow:hidden}.q4VXeG_captainState,.q4VXeG_memberState{color:var(--dsw-alias-label-tertiary);white-space:nowrap;flex:none;align-items:center;gap:5px;font-size:10px;font-weight:500;line-height:15px;display:inline-flex}.q4VXeG_captainState[data-busy=true],.q4VXeG_memberState[data-activity=working]{color:var(--dsw-alias-bg-fill-business)}.q4VXeG_workGlyph rect{opacity:.5}.q4VXeG_workGlyph[data-active=true] rect{animation:1.1s ease-in-out infinite q4VXeG_agentTeamsDot}@keyframes q4VXeG_agentTeamsDot{0%,to{opacity:.25}50%{opacity:1}}.q4VXeG_progressOverview{flex-direction:column;gap:7px;display:flex}.q4VXeG_progressTitle{color:var(--dsw-alias-label-secondary);font-size:11px;font-weight:600;line-height:16px}.q4VXeG_progressSegments{gap:3px;display:flex}.q4VXeG_progressSegments>span,.q4VXeG_progressEmpty{background:var(--dsw-alias-line-strong);border-radius:2px;flex:1;height:5px}.q4VXeG_progressEmpty{width:100%;display:block}.q4VXeG_progressSegments>span[data-state=running]{background:var(--dsw-alias-state-business-primary)}.q4VXeG_progressSegments>span[data-state=blocked]{background:var(--dsw-alias-state-warning)}.q4VXeG_progressSegments>span[data-state=completed]{background:var(--dsw-alias-state-success)}.q4VXeG_progressSegments>span[data-state=failed]{background:var(--dsw-alias-state-danger)}.q4VXeG_progressSegments>span[data-state=cancelled]{opacity:.55}.q4VXeG_progressLegend{color:var(--dsw-alias-label-tertiary);gap:10px;font-size:9.5px;line-height:14px;display:flex}.q4VXeG_progressLegend>span[data-state=running]{color:var(--dsw-alias-state-business-primary)}.q4VXeG_progressLegend>span[data-state=blocked]{color:var(--dsw-alias-state-warning)}.q4VXeG_progressLegend>span[data-state=completed]{color:var(--dsw-alias-state-success)}.q4VXeG_progressSummary{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 7%, var(--dsw-alias-bg-module));min-width:0;color:var(--dsw-alias-label-secondary);border-radius:8px;align-items:center;gap:6px;padding:5px 8px;font-size:10px;font-weight:600;line-height:15px;display:flex}.q4VXeG_progressSummary[data-state=warning]{background:color-mix(in srgb, var(--dsw-alias-state-warning) 8%, var(--dsw-alias-bg-module))}.q4VXeG_progressSummary[data-state=completed]{background:color-mix(in srgb, var(--dsw-alias-state-success) 8%, var(--dsw-alias-bg-module))}.q4VXeG_progressSummary[data-state=discarded]{background:var(--dsw-alias-bg-fill-neutral)}.q4VXeG_progressSummary>span:last-child{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.q4VXeG_progressSummaryDot{background:var(--dsw-alias-state-business-primary);border-radius:50%;flex:none;width:5px;height:5px}.q4VXeG_progressSummary[data-state=warning] .q4VXeG_progressSummaryDot{background:var(--dsw-alias-state-warning)}.q4VXeG_progressSummary[data-state=completed] .q4VXeG_progressSummaryDot{background:var(--dsw-alias-state-success)}.q4VXeG_progressSummary[data-state=discarded] .q4VXeG_progressSummaryDot{background:var(--dsw-alias-label-tertiary)}.q4VXeG_membersToggle{background:var(--dsw-alias-bg-module-platform);width:100%;color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;border:0;border-radius:8px;justify-content:space-between;align-items:center;gap:8px;padding:6px 8px;font-size:10.5px;font-weight:600;line-height:15px;display:flex}.q4VXeG_membersToggle:hover{background:var(--dsw-alias-bg-fill-neutral)}.q4VXeG_membersToggle>span{align-items:center;gap:5px;display:inline-flex}.q4VXeG_membersToggle>span:last-child{color:var(--dsw-alias-state-business-primary)}.q4VXeG_chevron{flex:none;transition:transform .14s}.q4VXeG_chevron[data-open=true]{transform:rotate(90deg)}.q4VXeG_delegationTree{flex-direction:column;gap:2px;margin-left:18px;padding:9px 0 0 20px;display:flex;position:relative}.q4VXeG_delegationTree:before{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 48%, var(--dsw-alias-line-normal));content:\"\";width:1px;position:absolute;top:0;bottom:22px;left:0}.q4VXeG_memberBlock{flex-direction:column;min-width:0;padding:3px 0 7px;display:flex;position:relative}.q4VXeG_memberBranch{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 48%, var(--dsw-alias-line-normal));width:20px;height:1px;display:block;position:absolute;top:27px;right:100%}.q4VXeG_memberBranch:before{background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:50%;width:5px;height:5px;position:absolute;top:-2px;right:-1px}.q4VXeG_memberRow{box-sizing:border-box;width:100%;min-width:0;min-height:48px;color:inherit;font:inherit;text-align:left;cursor:pointer;background:0 0;border:0;border-radius:8px;grid-template-columns:46px minmax(0,1fr) auto;align-items:center;gap:8px;padding:4px 6px;transition:background-color .12s,transform .12s;display:grid}.q4VXeG_memberRow:hover,.q4VXeG_memberRow[data-activity=working]{background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 6%, var(--dsw-alias-bg-module))}.q4VXeG_memberRow:active{transform:scale(.995)}.q4VXeG_memberAvatar{width:42px;height:42px}.q4VXeG_memberAvatar[data-unread=true]:after{box-sizing:border-box;border:1px solid var(--dsw-alias-bg-module);background:var(--dsw-alias-state-business-primary);content:\"\";border-radius:50%;width:6px;height:6px;animation:1.8s ease-in-out infinite q4VXeG_agentTeamsUnreadPulse;position:absolute;top:0;right:-1px}@keyframes q4VXeG_agentTeamsUnreadPulse{0%,to{opacity:.78;transform:scale(.92)}50%{opacity:1;transform:scale(1.16)}}.q4VXeG_memberInitial{background:var(--dsw-alias-bg-fill-business);width:34px;height:34px;color:var(--dsw-alias-label-on-fill);border-radius:50%;justify-content:center;align-items:center;font-size:14px;font-weight:600;line-height:20px;display:inline-flex}.q4VXeG_stateArt{box-sizing:border-box;object-fit:contain;width:22px;height:22px;filter:drop-shadow(0 0 1px var(--dsw-alias-bg-module)) drop-shadow(0 1px 1px #122d483d);background:0 0;border:0;border-radius:0;position:absolute;bottom:-3px;right:-5px}.q4VXeG_stateArt[data-activity=working]{animation:2.4s ease-in-out infinite q4VXeG_agentTeamsFloat}.q4VXeG_stateArt[data-activity=idle]{animation:4.2s ease-in-out infinite q4VXeG_agentTeamsBreathe}.q4VXeG_stateArt[data-activity=unknown]{animation:2.8s ease-in-out infinite q4VXeG_agentTeamsThink}@keyframes q4VXeG_agentTeamsFloat{0%,to{transform:translateY(0)rotate(-4deg)}50%{transform:translateY(-2px)rotate(4deg)}}@keyframes q4VXeG_agentTeamsBreathe{0%,to{opacity:.82;transform:scale(1)}50%{opacity:1;transform:scale(1.06)}}@keyframes q4VXeG_agentTeamsThink{0%,to{transform:rotate(-7deg)}50%{transform:rotate(7deg)}}.q4VXeG_memberState{margin-left:auto}.q4VXeG_memberCount{color:var(--dsw-alias-label-tertiary);font-size:10.5px;line-height:16px}.q4VXeG_assignmentLine{align-items:center;gap:7px;min-width:0;padding:0 6px 0 60px;display:flex}.q4VXeG_assignmentLabel{color:var(--dsw-alias-label-tertiary);flex:none;font-size:9.5px;line-height:14px}.q4VXeG_assignmentTasks{flex-wrap:wrap;flex:1;gap:4px;min-width:0;display:flex}.q4VXeG_assignmentChip{background:var(--dsw-alias-bg-fill-neutral);max-width:100%;min-height:16px;color:var(--dsw-alias-label-secondary);text-overflow:ellipsis;white-space:nowrap;border-radius:4px;align-items:center;padding:0 5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;font-weight:600;line-height:14px;display:inline-flex;overflow:hidden}.q4VXeG_assignmentChip[data-state=running]{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill)}.q4VXeG_assignmentChip[data-state=completed]{background:var(--dsw-alias-bg-fill-success);color:var(--dsw-alias-label-on-fill)}.q4VXeG_assignmentChip[data-state=blocked]{background:var(--dsw-alias-bg-fill-warning);color:var(--dsw-alias-label-on-fill)}.q4VXeG_assignmentChip[data-state=failed]{background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}.q4VXeG_assignmentChip[data-state=cancelled]{color:var(--dsw-alias-label-tertiary);text-decoration:line-through}.q4VXeG_unreadPill{color:var(--dsw-alias-state-business-primary);white-space:nowrap;flex:none;font-size:9.5px;font-weight:600;line-height:14px}.q4VXeG_taskEmpty{color:var(--dsw-alias-label-tertiary);font-size:9.5px;line-height:14px}.q4VXeG_dependencySection{border-top:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:7px;min-width:0;padding-top:10px;display:flex}.q4VXeG_sectionToggleTitle{color:var(--dsw-alias-label-secondary);font:inherit;cursor:pointer;background:0 0;border:0;align-items:center;gap:6px;padding:0;font-size:11px;font-weight:600;line-height:16px;display:inline-flex}.q4VXeG_dagViewport{scrollbar-width:thin;min-width:0;padding:2px 0 4px;overflow-x:auto}.q4VXeG_dagCanvas{min-width:100%;position:relative}.q4VXeG_dagCanvas[data-layout=parallel]{flex-wrap:wrap;gap:8px;display:flex}.q4VXeG_dagCanvas[data-layout=parallel] .q4VXeG_dagNode{flex:92px;min-width:92px;position:relative}.q4VXeG_dagEdges{pointer-events:none;position:absolute;inset:0;overflow:visible}.q4VXeG_dagEdges path{fill:none;stroke:var(--dsw-alias-line-strong);stroke-width:1px;transition:opacity .14s,stroke .14s,stroke-width .14s}.q4VXeG_dagEdges path[data-active=true]{stroke:var(--dsw-alias-state-business-primary);stroke-width:1.6px}.q4VXeG_dagEdges path[data-dimmed=true]{opacity:.24}.q4VXeG_dagNode{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module);color:var(--dsw-alias-label-primary);font:inherit;text-align:left;cursor:pointer;border-radius:6px;flex-direction:column;justify-content:center;gap:1px;padding:0 6px;transition:border-color .14s,background-color .14s,opacity .14s;display:flex;position:absolute}.q4VXeG_dagNode:hover,.q4VXeG_dagNode[data-focused=true]{border-color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 6%, var(--dsw-alias-bg-module))}.q4VXeG_dagNode[data-dimmed=true]{opacity:.3}.q4VXeG_dagNode[data-state=running][data-dimmed=true]{opacity:.58}.q4VXeG_dagNode[data-state=completed]{border-color:color-mix(in srgb, var(--dsw-alias-state-success) 48%, var(--dsw-alias-line-normal))}.q4VXeG_dagNode[data-state=blocked]{border-color:color-mix(in srgb, var(--dsw-alias-state-warning) 52%, var(--dsw-alias-line-normal))}.q4VXeG_dagNode[data-state=failed]{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 56%, var(--dsw-alias-line-normal))}.q4VXeG_dagNodeHead{color:var(--dsw-alias-label-primary);align-items:center;gap:4px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9.5px;font-weight:700;display:flex}.q4VXeG_dagNodeDot{background:var(--dsw-alias-line-strong);border-radius:1.5px;flex:none;width:5px;height:5px}.q4VXeG_dagNode[data-state=running] .q4VXeG_dagNodeDot{background:var(--dsw-alias-state-business-primary)}.q4VXeG_dagNode[data-state=running] .q4VXeG_dagNodeHead{padding-right:12px}.q4VXeG_dagRunningState{width:9px;height:9px;color:var(--dsw-alias-state-business-primary);pointer-events:none;justify-content:center;align-items:center;display:inline-flex;position:absolute;top:4px;right:5px}.q4VXeG_dagRunningState .q4VXeG_workGlyph{width:9px;height:9px}.q4VXeG_dagNode[data-state=blocked] .q4VXeG_dagNodeDot{background:var(--dsw-alias-state-warning)}.q4VXeG_dagNode[data-state=completed] .q4VXeG_dagNodeDot{background:var(--dsw-alias-state-success)}.q4VXeG_dagNode[data-state=failed] .q4VXeG_dagNodeDot{background:var(--dsw-alias-state-danger)}.q4VXeG_dagNodeLabel{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:11px;overflow:hidden}.q4VXeG_taskDetail{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:9px;flex-direction:column;gap:3px;min-width:0;padding:7px 9px;display:flex}.q4VXeG_taskDetailHead{align-items:center;gap:6px;min-width:0;display:flex}.q4VXeG_taskDetailId{color:var(--dsw-alias-state-business-primary);flex:none;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:10px;font-weight:700}.q4VXeG_taskDetailSubject{min-width:0;color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:11px;font-weight:600;line-height:16px;overflow:hidden}.q4VXeG_taskDetailBadge{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-secondary);border-radius:4px;flex:none;padding:0 5px;font-size:8.5px;font-weight:600;line-height:14px}.q4VXeG_taskDetailBadge[data-state=running]{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill)}.q4VXeG_taskDetailBadge[data-state=blocked]{background:var(--dsw-alias-bg-fill-warning);color:var(--dsw-alias-label-on-fill)}.q4VXeG_taskDetailBadge[data-state=completed]{background:var(--dsw-alias-bg-fill-success);color:var(--dsw-alias-label-on-fill)}.q4VXeG_taskDetailBadge[data-state=failed]{background:var(--dsw-alias-bg-fill-danger);color:var(--dsw-alias-label-on-fill)}.q4VXeG_taskDetailLine,.q4VXeG_taskDetailMeta{color:var(--dsw-alias-label-secondary);font-size:9.5px;line-height:14px}.q4VXeG_taskDetailMeta{color:var(--dsw-alias-label-tertiary)}.q4VXeG_emptyHint{color:var(--dsw-alias-label-tertiary);padding:10px 12px;font-size:11px;line-height:16px}.q4VXeG_planEditor{border:1px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 30%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 94%, var(--dsw-alias-state-business-primary));box-shadow:inset 0 1px 0 color-mix(in srgb, var(--dsw-alias-label-primary) 5%, transparent);border-radius:10px;flex-direction:column;gap:12px;margin:0 10px 12px;padding:12px;display:flex}.q4VXeG_planHeader>span{justify-content:space-between;align-items:center;gap:8px;display:flex}.q4VXeG_planHeader>span>span{flex-direction:column;gap:2px;min-width:0;display:flex}.q4VXeG_planHeader strong{color:var(--dsw-alias-label-primary);font-size:12px}.q4VXeG_planHeader small{color:var(--dsw-alias-label-secondary);font-size:9px;font-weight:500;line-height:13px}.q4VXeG_planHeader em{background:var(--dsw-alias-bg-fill-business);color:var(--dsw-alias-label-on-fill);border-radius:999px;flex:none;padding:1px 7px;font-size:9px;font-style:normal;line-height:16px}.q4VXeG_planHeader p{color:var(--dsw-alias-label-secondary);margin:5px 0 0;font-size:10px;line-height:15px}.q4VXeG_planFlow{grid-template-columns:repeat(3,minmax(0,1fr));margin:0;padding:0;list-style:none;display:grid}.q4VXeG_planFlow li{min-width:0;color:var(--dsw-alias-label-tertiary);align-items:center;gap:5px;font-size:9px;font-weight:600;line-height:14px;display:flex;position:relative}.q4VXeG_planFlow li:not(:last-child):after{background:var(--dsw-alias-line-normal);content:\"\";flex:1;min-width:8px;height:1px;margin-right:5px}.q4VXeG_planFlow li>span{border:1px solid var(--dsw-alias-line-normal);border-radius:50%;flex:none;place-items:center;width:18px;height:18px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:9px;display:grid}.q4VXeG_planFlow li[data-active]{color:var(--dsw-alias-state-business-primary)}.q4VXeG_planFlow li[data-active]>span{border-color:var(--dsw-alias-state-business-primary);background:color-mix(in srgb, var(--dsw-alias-state-business-primary) 12%, transparent)}.q4VXeG_planSection{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-module-platform);border-radius:8px;overflow:hidden}.q4VXeG_planSectionToggle,.q4VXeG_planCardHeader{box-sizing:border-box;width:100%;color:var(--dsw-alias-label-primary);cursor:pointer;text-align:left;background:0 0;border:0}.q4VXeG_planSectionToggle{justify-content:space-between;align-items:center;gap:8px;min-height:42px;padding:7px 9px;display:flex}.q4VXeG_planSectionToggle:hover,.q4VXeG_planCardHeader:hover{background:color-mix(in srgb, var(--dsw-alias-bg-fill-neutral) 46%, transparent)}.q4VXeG_planSectionToggle>span{align-items:baseline;gap:7px;min-width:0;display:flex}.q4VXeG_planSectionToggle strong{font-size:10.5px}.q4VXeG_planSectionToggle small{color:var(--dsw-alias-label-tertiary);font-size:9px}.q4VXeG_planList{border-top:1px solid var(--dsw-alias-line-normal);flex-direction:column;gap:0;display:flex}.q4VXeG_planEmpty{color:var(--dsw-alias-label-tertiary);text-align:center;margin:0;padding:12px;font-size:10px}.q4VXeG_planCard{background:0 0;border:0;border-radius:0;min-width:0;margin:0;padding:0;display:block;position:relative}.q4VXeG_planCard+.q4VXeG_planCard{border-top:1px solid var(--dsw-alias-line-normal)}.q4VXeG_planCard[data-open=true]{background:color-mix(in srgb, var(--dsw-alias-bg-base) 62%, transparent)}.q4VXeG_planCardHeader{grid-template-columns:minmax(80px,.9fr) minmax(72px,1.15fr) auto 12px;align-items:center;gap:7px;min-height:40px;padding:6px 9px;display:grid}.q4VXeG_planCardIdentity{flex-direction:column;gap:1px;min-width:0;display:flex}.q4VXeG_planCardIdentity strong,.q4VXeG_planTaskSummary{color:var(--dsw-alias-label-primary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;font-weight:650;line-height:14px;overflow:hidden}.q4VXeG_planCardIdentity>span,.q4VXeG_planCardMeta{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:12px;overflow:hidden}.q4VXeG_planTaskId{background:var(--dsw-alias-bg-fill-neutral);width:max-content;color:var(--dsw-alias-label-secondary);border-radius:4px;padding:1px 5px;font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:8.5px;font-weight:700;line-height:14px}.q4VXeG_planDirty{background:color-mix(in srgb, var(--dsw-alias-state-warning) 13%, transparent);color:var(--dsw-alias-state-warning);border-radius:999px;justify-self:end;padding:1px 5px;font-size:8px;font-style:normal;font-weight:650;line-height:14px}.q4VXeG_planChevron{color:var(--dsw-alias-label-tertiary);flex:none;transition:transform .18s cubic-bezier(.2,.7,.2,1)}.q4VXeG_planChevron[data-open=true]{transform:rotate(90deg)}.q4VXeG_planCardBody{flex-direction:column;gap:8px;padding:0 9px 9px;display:flex}.q4VXeG_planCardBody fieldset{border:0;flex-direction:column;gap:7px;min-width:0;margin:0;padding:0;display:flex}.q4VXeG_planCardBody label,.q4VXeG_planNewTask label{min-width:0;color:var(--dsw-alias-label-tertiary);flex-direction:column;flex:1;gap:4px;font-size:9px;display:flex}.q4VXeG_planCardBody label small{color:var(--dsw-alias-label-tertiary);font-size:8px;line-height:11px}.q4VXeG_planCard input,.q4VXeG_planCard textarea,.q4VXeG_planCard select,.q4VXeG_planNewTask input{box-sizing:border-box;border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-base);width:100%;min-width:0;color:var(--dsw-alias-label-primary);font:inherit;border-radius:6px;outline:none;font-size:10.5px;line-height:16px;transition:border-color .16s,box-shadow .16s}.q4VXeG_planCard input,.q4VXeG_planCard select,.q4VXeG_planNewTask input{min-height:32px;padding:6px 8px}.q4VXeG_planCard textarea{resize:vertical;min-height:58px;padding:7px 8px}.q4VXeG_planCard input:focus-visible,.q4VXeG_planCard textarea:focus-visible,.q4VXeG_planCard select:focus-visible,.q4VXeG_planNewTask input:focus-visible{border-color:var(--dsw-alias-state-business-primary);box-shadow:0 0 0 2px color-mix(in srgb, var(--dsw-alias-state-business-primary) 16%, transparent)}.q4VXeG_planGrid{grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:6px;display:grid}.q4VXeG_planModelPicker{grid-template-columns:minmax(0,1fr);gap:5px;display:grid}.q4VXeG_planModelMenu{width:100%;display:flex}.q4VXeG_planModelTrigger{border:1px solid var(--dsw-alias-border-l2);background:var(--dsw-alias-bg-base);width:100%;min-height:38px;color:var(--dsw-alias-label-primary);cursor:pointer;text-align:left;border-radius:7px;justify-content:space-between;align-items:center;gap:8px;padding:7px 9px;transition:border-color .16s,background-color .16s,transform .12s;display:flex}.q4VXeG_planModelTrigger:hover:not(:disabled){border-color:var(--dsw-alias-border-l3);background:var(--dsw-alias-interactive-bg-hover)}.q4VXeG_planModelTrigger:active:not(:disabled){transform:translateY(1px)}.q4VXeG_planModelTrigger:focus-visible{border-color:var(--dsw-alias-state-business-primary);outline:2px solid color-mix(in srgb, var(--dsw-alias-state-business-primary) 16%, transparent);outline-offset:1px}.q4VXeG_planModelTrigger:disabled{cursor:wait;opacity:.64}.q4VXeG_planModelTriggerCopy{align-items:baseline;gap:6px;min-width:0;display:flex}.q4VXeG_planModelTriggerCopy strong,.q4VXeG_planModelTriggerCopy span{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.q4VXeG_planModelTriggerCopy strong{color:var(--dsw-alias-label-primary);font-size:10px;font-weight:650;line-height:15px}.q4VXeG_planModelTriggerCopy span{color:var(--dsw-alias-label-tertiary);font-size:9px;line-height:14px}.q4VXeG_planModelMenuRow{grid-template-columns:auto minmax(0,1fr) auto;align-items:center;gap:8px;width:100%;min-width:0;display:grid}.q4VXeG_planModelMenuRow>span:first-child{color:var(--dsw-alias-label-primary)}.q4VXeG_planModelMenuRow strong{color:var(--dsw-alias-label-tertiary);text-align:right;text-overflow:ellipsis;white-space:nowrap;font-weight:450;overflow:hidden}.q4VXeG_planModelMenuBack{align-items:center;gap:7px;display:inline-flex}.q4VXeG_planModelMenuBack svg{transform:rotate(180deg)}.q4VXeG_planModelEffortRow{flex-direction:column;align-items:flex-start;min-width:0;display:flex}.q4VXeG_planModelEffortRow small{width:100%;color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:10px;line-height:14px;overflow:hidden}.q4VXeG_planModelHint{color:var(--dsw-alias-label-tertiary);text-overflow:ellipsis;white-space:nowrap;font-size:8.5px;line-height:12px;overflow:hidden}.q4VXeG_planModelNotice{background:color-mix(in srgb, var(--dsw-alias-state-warning) 9%, transparent);color:var(--dsw-alias-label-secondary);border-radius:6px;grid-column:1/-1;justify-content:space-between;align-items:center;gap:8px;padding:6px 7px;font-size:8.5px;line-height:12px;display:flex}.q4VXeG_planModelNotice button{color:var(--dsw-alias-state-business-primary);cursor:pointer;font:inherit;background:0 0;border:0;flex:none;padding:2px 6px;font-weight:650}.q4VXeG_planActions,.q4VXeG_planApproveRow,.q4VXeG_planNewTask,.q4VXeG_planConfirm,.q4VXeG_planApproveActions,.q4VXeG_planSecondaryActions{align-items:center;gap:7px;display:flex}.q4VXeG_planReviewActions{grid-template-columns:minmax(0,1fr);gap:6px;width:100%;display:grid}.q4VXeG_planSecondaryActions{grid-template-columns:minmax(0,1fr) auto;display:grid}.q4VXeG_planActions{justify-content:flex-end}.q4VXeG_planActions button,.q4VXeG_planNewTask button,.q4VXeG_planApproveRow button,.q4VXeG_planConfirm button{border:1px solid var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-fill-neutral);min-height:30px;color:var(--dsw-alias-label-primary);cursor:pointer;border-radius:6px;flex:none;padding:5px 10px;font-size:9.5px;font-weight:600;transition:background .16s,border-color .16s,transform .16s}.q4VXeG_planActions button:hover:not(:disabled),.q4VXeG_planNewTask button:hover:not(:disabled),.q4VXeG_planApproveRow button:hover:not(:disabled),.q4VXeG_planConfirm button:hover:not(:disabled){border-color:var(--dsw-alias-label-tertiary)}.q4VXeG_planActions button:active:not(:disabled),.q4VXeG_planNewTask button:active:not(:disabled),.q4VXeG_planApproveRow button:active:not(:disabled),.q4VXeG_planConfirm button:active:not(:disabled){transform:scale(.98)}.q4VXeG_planActions button[data-danger],.q4VXeG_planConfirm button[data-danger]{color:var(--dsw-alias-state-danger)}.q4VXeG_planFeedback{min-width:0;color:var(--dsw-alias-label-secondary);flex:1;align-items:center;gap:5px;font-size:9px;line-height:13px;animation:.18s ease-out q4VXeG_plan-feedback-in;display:inline-flex}.q4VXeG_planFeedback[data-tone=success]{color:var(--dsw-alias-state-success)}.q4VXeG_planFeedback[data-tone=error]{color:var(--dsw-alias-state-danger)}.q4VXeG_planFeedback>span{border:1px solid;border-radius:50%;flex:none;place-items:center;width:15px;height:15px;display:grid}.q4VXeG_planFeedback svg{width:11px;height:11px}@keyframes q4VXeG_plan-feedback-in{0%{opacity:0;transform:translateY(-2px)}to{opacity:1;transform:translateY(0)}}.q4VXeG_planConfirm{border:1px solid color-mix(in srgb, var(--dsw-alias-state-danger) 30%, var(--dsw-alias-line-normal));background:color-mix(in srgb, var(--dsw-alias-state-danger) 7%, transparent);border-radius:7px;flex-wrap:wrap;justify-content:flex-end;padding:7px}.q4VXeG_planConfirm>span{min-width:140px;color:var(--dsw-alias-label-secondary);flex:1;font-size:9px;line-height:13px}.q4VXeG_planNewTask{align-items:flex-end}.q4VXeG_planNewTask label{gap:4px}.q4VXeG_planNewTask label>span{line-height:13px}.q4VXeG_planApproveRow{z-index:1;border:1px solid var(--dsw-alias-line-normal);background:color-mix(in srgb, var(--dsw-alias-bg-module-platform) 94%, transparent);min-height:50px;box-shadow:0 -5px 16px color-mix(in srgb, var(--dsw-alias-bg-base) 35%, transparent);backdrop-filter:blur(8px);border-radius:8px;flex-direction:column;justify-content:flex-end;align-items:stretch;margin:0 -4px -4px;padding:8px;position:sticky;bottom:0}.q4VXeG_planApproveRow[data-armed=true]{border-color:color-mix(in srgb, var(--dsw-alias-state-business-primary) 45%, var(--dsw-alias-line-normal))}.q4VXeG_planApproveRow[data-discard=true]{border-color:color-mix(in srgb, var(--dsw-alias-state-danger) 45%, var(--dsw-alias-line-normal))}.q4VXeG_planApproveCopy{flex-direction:column;flex:1;gap:2px;min-width:0;display:flex}.q4VXeG_planApproveCopy strong{color:var(--dsw-alias-label-primary);font-size:9.5px;line-height:13px}.q4VXeG_planApproveCopy small{color:var(--dsw-alias-label-tertiary);font-size:8.5px;line-height:12px}.q4VXeG_planApproveRow button{background:var(--dsw-alias-state-business-primary);min-height:32px;color:var(--dsw-alias-label-on-fill);padding-inline:13px}.q4VXeG_planReviewActions>button[data-plan-approve]{width:100%}.q4VXeG_planApproveActions>button:first-child{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}.q4VXeG_planSecondaryActions>button,.q4VXeG_planApproveActions>button[data-danger]{border-color:var(--dsw-alias-line-normal);background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-primary)}.q4VXeG_planSecondaryActions>button[data-danger],.q4VXeG_planApproveActions>button[data-danger]{color:var(--dsw-alias-state-danger)}.q4VXeG_planSectionToggle:focus-visible,.q4VXeG_planCardHeader:focus-visible,.q4VXeG_planActions button:focus-visible,.q4VXeG_planNewTask button:focus-visible,.q4VXeG_planApproveRow button:focus-visible,.q4VXeG_planConfirm button:focus-visible{outline:2px solid var(--dsw-alias-state-business-primary);outline-offset:-2px}.q4VXeG_planActions button:disabled,.q4VXeG_planNewTask button:disabled,.q4VXeG_planApproveRow button:disabled{cursor:default;opacity:.55}.q4VXeG_historicPill{background:var(--dsw-alias-bg-fill-neutral);color:var(--dsw-alias-label-tertiary);border-radius:4px;flex:none;margin-left:auto;padding:1px 7px;font-size:9.5px;font-weight:600;line-height:15px}.q4VXeG_members{flex-direction:column;gap:3px;display:flex}.q4VXeG_archiveLabel{color:var(--dsw-alias-label-tertiary);padding:5px 14px 0;font-size:9.5px;font-weight:600;line-height:14px;display:block}@media (prefers-reduced-motion:reduce){.q4VXeG_panel,.q4VXeG_badge,.q4VXeG_badgeDot,.q4VXeG_panelDot,.q4VXeG_waterWaves,.q4VXeG_waterWaves:before,.q4VXeG_waterWaves:after,.q4VXeG_workGlyph rect,.q4VXeG_stateArt,.q4VXeG_memberAvatar[data-unread=true]:after,.q4VXeG_planChevron,.q4VXeG_planFeedback,.q4VXeG_planActions button,.q4VXeG_planNewTask button,.q4VXeG_planApproveRow button,.q4VXeG_planConfirm button,.q4VXeG_planCard input,.q4VXeG_planCard textarea,.q4VXeG_planCard select,.q4VXeG_planNewTask input{transition:none;animation:none}}@media (width<=960px){html[data-agent-teams-panel-open] [data-phase=active]{padding-right:0}}@media (width<=640px){.q4VXeG_badge{top:56px;right:10px}.q4VXeG_teamStats span[data-stat=messages]{display:none}.q4VXeG_captainNode{grid-template-columns:48px minmax(0,1fr)}.q4VXeG_captainState{display:none}.q4VXeG_delegationTree{margin-left:12px;padding-left:15px}.q4VXeG_memberBranch{width:15px}.q4VXeG_assignmentLine{padding-left:53px}.q4VXeG_planFlow li{gap:4px;font-size:8px}.q4VXeG_planFlow li:not(:last-child):after{margin-right:3px}.q4VXeG_planCardHeader{grid-template-columns:auto minmax(0,1fr) auto}.q4VXeG_planCardHeader .q4VXeG_planCardMeta{display:none}.q4VXeG_planGrid,.q4VXeG_planModelPicker{grid-template-columns:minmax(0,1fr)}.q4VXeG_planNewTask,.q4VXeG_planApproveRow{flex-direction:column;align-items:stretch}.q4VXeG_planNewTask button,.q4VXeG_planApproveRow>button,.q4VXeG_planApproveActions,.q4VXeG_planReviewActions{width:100%}.q4VXeG_planApproveActions button,.q4VXeG_planReviewActions button,.q4VXeG_planSecondaryActions button{flex:1}}@container q4VXeG_agent-team (width<=360px){.q4VXeG_planEditor{margin-inline:0;padding-inline:10px}.q4VXeG_planHeader>span{align-items:flex-start}.q4VXeG_planFlow li{gap:3px;font-size:7.5px}.q4VXeG_planFlow li:not(:last-child):after{min-width:4px;margin-right:2px}.q4VXeG_planSecondaryActions,.q4VXeG_planApproveActions{grid-template-columns:minmax(0,1fr);width:100%;display:grid}.q4VXeG_planSecondaryActions button,.q4VXeG_planApproveActions button{width:100%}}";
		const tagId = "dsh-dai-agent-teams/ActivityPanel.module.css";
		...
		var ActivityPanel_module_css_default = {
			"agent-team": "q4VXeG_agent-team",
			"agentTeamsBadgeWave": "q4VXeG_agentTeamsBadgeWave",
			"agentTeamsBreathe": "q4VXeG_agentTeamsBreathe",
			"agentTeamsDot": "q4VXeG_agentTeamsDot",
			"agentTeamsFloat": "q4VXeG_agentTeamsFloat",
			"agentTeamsPanelIn": "q4VXeG_agentTeamsPanelIn",
			"agentTeamsPulse": "q4VXeG_agentTeamsPulse",
			"agentTeamsThink": "q4VXeG_agentTeamsThink",
			"agentTeamsUnreadPulse": "q4VXeG_agentTeamsUnreadPulse",
			"agentTeamsWave": "q4VXeG_agentTeamsWave",
			"archiveLabel": "q4VXeG_archiveLabel",
			"assignmentChip": "q4VXeG_assignmentChip",
			"assignmentLabel": "q4VXeG_assignmentLabel",
			"assignmentLine": "q4VXeG_assignmentLine",
			"assignmentTasks": "q4VXeG_assignmentTasks",
			"badge": "q4VXeG_badge",
			"badgeCount": "q4VXeG_badgeCount",
			"badgeDot": "q4VXeG_badgeDot",
			"badgeText": "q4VXeG_badgeText",
			"captainAvatar": "q4VXeG_captainAvatar",
			"captainInfo": "q4VXeG_captainInfo",
			"captainLine": "q4VXeG_captainLine",
			"captainName": "q4VXeG_captainName",
			"captainNode": "q4VXeG_captainNode",
			"captainRole": "q4VXeG_captainRole",
			"captainState": "q4VXeG_captainState",
			"captainSummary": "q4VXeG_captainSummary",
			"chevron": "q4VXeG_chevron",
			"dagCanvas": "q4VXeG_dagCanvas",
			"dagEdges": "q4VXeG_dagEdges",
			"dagNode": "q4VXeG_dagNode",
			"dagNodeDot": "q4VXeG_dagNodeDot",
			"dagNodeHead": "q4VXeG_dagNodeHead",
			"dagNodeLabel": "q4VXeG_dagNodeLabel",
			"dagRunningState": "q4VXeG_dagRunningState",
			"dagViewport": "q4VXeG_dagViewport",
			"delegationSection": "q4VXeG_delegationSection",
			"delegationTree": "q4VXeG_delegationTree",
			"dependencySection": "q4VXeG_dependencySection",
			"emptyHint": "q4VXeG_emptyHint",
			"historicPill": "q4VXeG_historicPill",
			"iconButton": "q4VXeG_iconButton",
			"leadAvatar": "q4VXeG_leadAvatar",
			"memberArt": "q4VXeG_memberArt",
			"memberAvatar": "q4VXeG_memberAvatar",
			"memberBlock": "q4VXeG_memberBlock",
			"memberBranch": "q4VXeG_memberBranch",
			"memberCount": "q4VXeG_memberCount",
			"memberInfo": "q4VXeG_memberInfo",
			"memberInitial": "q4VXeG_memberInitial",
			"memberLine": "q4VXeG_memberLine",
			"memberModel": "q4VXeG_memberModel",
			"memberName": "q4VXeG_memberName",
			"memberRole": "q4VXeG_memberRole",
			"memberRow": "q4VXeG_memberRow",
			"memberState": "q4VXeG_memberState",
			"memberStatusLine": "q4VXeG_memberStatusLine",
			"members": "q4VXeG_members",
			"membersToggle": "q4VXeG_membersToggle",
			"panel": "q4VXeG_panel",
			"panelControls": "q4VXeG_panelControls",
			"panelDot": "q4VXeG_panelDot",
			"panelHead": "q4VXeG_panelHead",
			"panelTitle": "q4VXeG_panelTitle",
			"plan-feedback-in": "q4VXeG_plan-feedback-in",
			"planActions": "q4VXeG_planActions",
			"planApproveActions": "q4VXeG_planApproveActions",
			"planApproveCopy": "q4VXeG_planApproveCopy",
			"planApproveRow": "q4VXeG_planApproveRow",
			"planCard": "q4VXeG_planCard",
			"planCardBody": "q4VXeG_planCardBody",
			"planCardHeader": "q4VXeG_planCardHeader",
			"planCardIdentity": "q4VXeG_planCardIdentity",
			"planCardMeta": "q4VXeG_planCardMeta",
			"planChevron": "q4VXeG_planChevron",
			"planConfirm": "q4VXeG_planConfirm",
			"planDirty": "q4VXeG_planDirty",
			"planEditor": "q4VXeG_planEditor",
			"planEmpty": "q4VXeG_planEmpty",
			"planFeedback": "q4VXeG_planFeedback",
			"planFlow": "q4VXeG_planFlow",
			"planGrid": "q4VXeG_planGrid",
			"planHeader": "q4VXeG_planHeader",
			"planList": "q4VXeG_planList",
			"planModelEffortRow": "q4VXeG_planModelEffortRow",
			"planModelHint": "q4VXeG_planModelHint",
			"planModelMenu": "q4VXeG_planModelMenu",
			"planModelMenuBack": "q4VXeG_planModelMenuBack",
			"planModelMenuRow": "q4VXeG_planModelMenuRow",
			"planModelNotice": "q4VXeG_planModelNotice",
			"planModelPicker": "q4VXeG_planModelPicker",
			"planModelTrigger": "q4VXeG_planModelTrigger",
			"planModelTriggerCopy": "q4VXeG_planModelTriggerCopy",
			"planNewTask": "q4VXeG_planNewTask",
			"planReviewActions": "q4VXeG_planReviewActions",
			"planSecondaryActions": "q4VXeG_planSecondaryActions",
			"planSection": "q4VXeG_planSection",
			"planSectionToggle": "q4VXeG_planSectionToggle",
			"planTaskId": "q4VXeG_planTaskId",
			"planTaskSummary": "q4VXeG_planTaskSummary",
			"progressEmpty": "q4VXeG_progressEmpty",
			"progressLegend": "q4VXeG_progressLegend",
			"progressOverview": "q4VXeG_progressOverview",
			"progressSegments": "q4VXeG_progressSegments",
			"progressSummary": "q4VXeG_progressSummary",
			"progressSummaryDot": "q4VXeG_progressSummaryDot",
			"progressTitle": "q4VXeG_progressTitle",
			"resizeHandle": "q4VXeG_resizeHandle",
			"sectionHead": "q4VXeG_sectionHead",
			"sectionHint": "q4VXeG_sectionHint",
			"sectionTitle": "q4VXeG_sectionTitle",
			"sectionToggleTitle": "q4VXeG_sectionToggleTitle",
			"stageLabel": "q4VXeG_stageLabel",
			"stateArt": "q4VXeG_stateArt",
			"stopModalActions": "q4VXeG_stopModalActions",
			"stopModalError": "q4VXeG_stopModalError",
			"taskDetail": "q4VXeG_taskDetail",
			"taskDetailBadge": "q4VXeG_taskDetailBadge",
			"taskDetailHead": "q4VXeG_taskDetailHead",
			"taskDetailId": "q4VXeG_taskDetailId",
			"taskDetailLine": "q4VXeG_taskDetailLine",
			"taskDetailMeta": "q4VXeG_taskDetailMeta",
			"taskDetailModel": "q4VXeG_taskDetailModel",
			"taskDetailSubject": "q4VXeG_taskDetailSubject",
			"taskEmpty": "q4VXeG_taskEmpty",
			"taskId": "q4VXeG_taskId",
			"team": "q4VXeG_team",
			"teamHead": "q4VXeG_teamHead",
			"teamName": "q4VXeG_teamName",
			"teamStats": "q4VXeG_teamStats",
			"teamStopButton": "q4VXeG_teamStopButton",
			"teams": "q4VXeG_teams",
			"unreadPill": "q4VXeG_unreadPill",
			"waterWaves": "q4VXeG_waterWaves",
			"workGlyph": "q4VXeG_workGlyph"
		};
		//#endregion
		...
		*/
		...
		const PANEL_BADGE_OFFSET_STORAGE_KEY = "dsh-agent-teams:activity-badge-offset:v1";
		...
		* claimed/pending/failed/cancelled keep their own labels and colors. */
		...
		function CollapsedBadge({ count, phase, busy, offset, onDrag, onDragEnd, onClick, t, dragging }) {
			...
			return (0, react_jsx_runtime.jsxs)("button", {
				...
				children: [
					...
					(0, react_jsx_runtime.jsx)("span", {
						className: ActivityPanel_module_css_default.badgeText,
						children: t(`activity.phase.${phase}`)
					}),
					(0, react_jsx_runtime.jsx)("span", {
		...
		* durable roster through the same rebuilt panel instead of a second UI. */
		...
		function ActivityPanel({ sessionsList, modelDirectories, openMember, t }) {
			...
			const phase = (0, react.useMemo)(() => panelPhaseOf(visibleTeams, visibleArchived), [visibleTeams, visibleArchived]);
			const activePhase = phase === "idle" ? "running" : phase;
			const runsWater = panelRunsWater(phase);
				...
				children: [
					(0, react_jsx_runtime.jsxs)("header", {
						...
						children: [(0, react_jsx_runtime.jsxs)("span", {
							...
							children: [t(`activity.phase.${activePhase}`), (0, react_jsx_runtime.jsx)("span", {
					...
					(0, react_jsx_runtime.jsx)("div", {
						className: ActivityPanel_module_css_default.waterWaves,
		...
		*/
		...
		const zh = {
			...
			"activity.phase.planning": "AgentTeams 规划中",
			"activity.phase.running": "AgentTeams 运行中",
			"activity.phase.completed": "AgentTeams 已完成",
			"activity.phase.failed": "AgentTeams 失败",
			"activity.phase.halted": "AgentTeams 已中断",
		...
		const en = {
			...
			"activity.phase.planning": "AgentTeams planning",
			"activity.phase.running": "AgentTeams running",
			"activity.phase.completed": "AgentTeams completed",
			"activity.phase.failed": "AgentTeams failed",
			"activity.phase.halted": "AgentTeams interrupted",
