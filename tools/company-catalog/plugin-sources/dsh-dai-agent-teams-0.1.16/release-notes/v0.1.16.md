# AgentTeams v0.1.16 — stable release

This is the first stable release of AgentTeams on npm `latest`. It restores team execution across the exact Harness targets `0.1.2-rc.1`, `0.1.2-alpha.5`, and `0.1.2-alpha.2`, and passes the full Ubuntu and Windows verification matrix. Install the exact plugin version with `dsh plugin --profile web add --save-exact dsh-dai-agent-teams@0.1.16` after checking that the running host is one of those targets. Older Desktop cores require their own compatible pair.
...
- Fix the activity badge so a completed team no longer stays stuck showing "running": the panel phase now falls back to archived teams when no live team remains, so a finished team is reported as completed instead of running.
...
- Use `next` for plugin prereleases. A stable `latest` requires the recommended host and complete matrix; Alpha-only validation is insufficient. `0.1.16` is the first stable release that satisfies that gate.
...
See the repository's [maintenance workflow](../docs/maintenance-workflow.md) and the dated implementation/acceptance record for the exact outcomes and remaining limits. Older `0.1.0-*` and `0.1.1-*` hosts are outside this release's support matrix; retain a known compatible exact pair or migrate both sides.
