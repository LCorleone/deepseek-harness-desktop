...
[v0.1.16](./release-notes/v0.1.16.md) is the first stable release on npm `latest`, with fixes for startup, member messaging and task collaboration across supported Harness RC / Alpha versions. Choose a version pair below.
...
**Recommended pair: DeepSeek Harness `0.1.2-rc.1` + AgentTeams `0.1.16`.** Harness is still a prerelease; the plugin is stable.

| Use case | DeepSeek Harness | AgentTeams plugin |
| --- | --- | --- |
| **Recommended installation** | **`0.1.2-rc.1`** | **`0.1.16`** |
| Developer Alpha testing | `0.1.2-alpha.5` | `0.1.16` |
| Retaining an older Alpha | `0.1.2-alpha.2` | `0.1.16` |
...
```sh
dsh plugin --profile web add --save-exact dsh-dai-agent-teams@0.1.16
```

**After installation, stop and restart Harness for that profile, then refresh the browser.**

The stable plugin `0.1.16` is published on `latest`; `0.1.15` targets Alpha.2. Use the exact-version command above.
