...
function crc32Of(data) {
  ...
  for (let index = 0; index < data.length; index += 1) {
    state = crcFold(state, data[index]);
  }
  return (state ^ 4294967295) >>> 0;
}
function buildBinaryZip(parts) {
  if (parts.length === 0) throw new Error("cannot build an empty zip archive");
  const chunks = [];
  const central = [];
  let offset = 0;
  for (const part of parts) {
...
function binaryPartsOf(zip) {
  return zip.entryNames().map((name2) => ({ name: name2, data: zip.entryData(name2) }));
}
...
async function saveOfficeBinary(exec, ctx, target, bytes) {
  exec.signal.throwIfAborted();
  const absolute = ctx.fs.processPath(target);
  const staging = `${absolute}.office-${process.pid}-${randomBytes(4).toString("hex")}.tmp`;
  try {
    await fsWriteFile(staging, bytes);
    exec.signal.throwIfAborted();
    await fsRename(staging, absolute);
  } catch (error) {
    await fsUnlink(staging).catch(() => {});
    throw error;
  }
  exec.signal.throwIfAborted();
...
var XLSX_APP_PROPS = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>dsh-dai-office-tools</Application></Properties>';
...
function contentTypesXml(slideCount, notesCount) {
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Types ${CT_NS}><Default Extension="rels" ContentType="application/vnd.openxmlformats-package.relationships+xml"/><Default Extension="xml" ContentType="application/xml"/><Default Extension="svg" ContentType="image/svg+xml"/><Override PartName="/ppt/presentation.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.presentation.main+xml"/><Override PartName="/ppt/slideMasters/slideMaster1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideMaster+xml"/><Override PartName="/ppt/slideLayouts/slideLayout1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slideLayout+xml"/><Override PartName="/ppt/notesMasters/notesMaster1.xml" ContentType="application/vnd.openxmlformats-officedocument.presentationml.notesMaster+xml"/><Override PartName="/ppt/theme/theme1.xml" ContentType="application/vnd.openxmlformats-officedocument.theme+xml"/><Override PartName="/ppt/theme/theme2.xml" ContentType="application/vnd.openxmlformats-officedocument.theme+xml"/>` + slides + notes + '<Override PartName="/docProps/core.xml" ContentType="application/vnd.openxmlformats-package.core-properties+xml"/><Override PartName="/docProps/app.xml" ContentType="application/vnd.openxmlformats-officedocument.extended-properties+xml"/></Types>';
}
...
var APP_PROPS_XML = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>dsh-dai-office-tools</Application><PresentationFormat>Widescreen</PresentationFormat></Properties>';
...
function slideRelsXml(index, imageTargets, hasNotes, layoutTarget = "../slideLayouts/slideLayout1.xml") {
  const images = imageTargets.map((target, offset) => relationshipXml(`rImg${offset + 1}`, "officeDocument/2006/relationships/image", target, false)).join("");
  ...
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships ${REL_NS}>` + relationshipXml("rId1", "officeDocument/2006/relationships/slideLayout", layoutTarget, false) + images + notes + "</Relationships>";
}
function notesRelsXml(index, notesMasterTarget = "../notesMasters/notesMaster1.xml") {
  return `<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Relationships ${REL_NS}>` + relationshipXml("rId1", "officeDocument/2006/relationships/slide", `../slides/slide${index}.xml`, false) + relationshipXml("rId2", "officeDocument/2006/relationships/notesMaster", notesMasterTarget, false) + "</Relationships>";
}
...
function embeddedSvgPart(bytes, mime, pixelWidth, pixelHeight) {
  const width = pixelWidth ?? 1;
  const height = pixelHeight ?? 1;
  const base64 = Buffer.from(bytes).toString("base64");
  return `<?xml version="1.0" encoding="UTF-8" standalone="no"?><svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="${width}" height="${height}" viewBox="0 0 ${width} ${height}"><image x="0" y="0" width="${width}" height="${height}" preserveAspectRatio="none" xlink:href="data:${mime};base64,${base64}"/></svg>`;
}
async function placeImage(exec, ctx, deck, image, slideIndex, imageIndex, mediaName) {
  void deck;
  ...
  const bytes = await ctx.fs.readBytes(resolved.target, exec.signal, MAX_IMAGE_BYTES);
  const intrinsic = sniffImageSize(bytes.subarray(0, Math.min(bytes.byteLength, 1024)));
  const target = mediaName ?? `ppt/media/image${slideIndex * MAX_IMAGES_PER_SLIDE + imageIndex + 1}.svg`;
  const media = embeddedSvgPart(bytes, imageMimeType(resolved.ext), intrinsic?.width, intrinsic?.height);
...
function slideParts(build) {
  ...
  if (images.length > 0) {
    ...
    images.forEach((image, imageIndex) => {
      ...
      shapes.push(embeddedPicturePart(id++, image, `rImg${imageIndex + 1}`));
      const { target: _target, mediaName: _mediaName, media: _media, pixelWidth: _w, pixelHeight: _h, crop: _c, ...box } = image;
...
async function buildPptxText(args, exec, ctx, deck) {
  ...
  for (const [slideIndex, build] of builds.entries()) {
    ...
    build.images = [];
    for (const [imageIndex, image] of images.entries()) {
      const mediaName = `ppt/media/image${slideIndex * MAX_IMAGES_PER_SLIDE + imageIndex + 1}.svg`;
      build.images.push(await placeImage(exec, ctx, deck, image, slideIndex, imageIndex, mediaName));
    }
  }
  ...
  builds.forEach((build, index) => {
    ...
    parts.push({
      ...
      content: slideRelsXml(number, build.images.map((image) => `../media/${image.mediaName.split("/").at(-1)}`), hasNotes)
    });
    for (const image of build.images) {
      parts.push({ name: image.mediaName, content: image.media });
    }
    if (hasNotes) {
...
function maxNumberInXml(xml, pattern) {
  let maximum = 0;
...
function maxSlideFileNumber(zip) {
  return zip.entryNames().reduce((maximum, name2) => Math.max(maximum, slideNumber(name2)), 0);
}
function maxMediaFileNumber(zip) {
  let maximum = 0;
  for (const name2 of zip.entryNames()) {
    const match = name2.match(/^ppt\/media\/image(\d+)\.[^/]+$/);
    if (match !== null) maximum = Math.max(maximum, Number.parseInt(match[1], 10));
  }
  return maximum;
}
function nextRelationshipId(xml, used = /* @__PURE__ */ new Set()) {
  for (const match of xml.matchAll(/\bId="rId(\d+)"/g)) used.add(Number.parseInt(match[1], 10));
  let value = 1;
  while (used.has(value)) value += 1;
  used.add(value);
  return `rId${value}`;
}
function relationshipTarget(xml, relationshipName, fallback) {
  const pattern = new RegExp(`<Relationship\\b[^>]*Type="[^"]*\\/${relationshipName}"[^>]*Target="([^"]+)"`);
  return xml.match(pattern)?.[1] ?? fallback;
}
function appendBeforeClose(xml, closeTag, addition, label) {
  const index = xml.lastIndexOf(closeTag);
  if (index === -1) throw new Error(`${label} has no ${closeTag}`);
  return `${xml.slice(0, index)}${addition}${xml.slice(index)}`;
}
function appendPptContentTypes(xml, slideNumbers, notesNumbers, needsSvg) {
  let updated = xml;
  if (needsSvg && !/<Default\b[^>]*Extension="svg"/.test(updated)) {
    updated = appendBeforeClose(updated, "</Types>", '<Default Extension="svg" ContentType="image/svg+xml"/>', "[Content_Types].xml");
  }
  const overrides = [];
  for (const number of slideNumbers) {
    const partName = `/ppt/slides/slide${number}.xml`;
    if (!updated.includes(`PartName="${partName}"`)) {
      overrides.push(`<Override PartName="${partName}" ContentType="application/vnd.openxmlformats-officedocument.presentationml.slide+xml"/>`);
    }
  }
  for (const number of notesNumbers) {
    const partName = `/ppt/notesSlides/notesSlide${number}.xml`;
    if (!updated.includes(`PartName="${partName}"`)) {
      overrides.push(`<Override PartName="${partName}" ContentType="application/vnd.openxmlformats-officedocument.presentationml.notesSlide+xml"/>`);
    }
  }
  return overrides.length === 0 ? updated : appendBeforeClose(updated, "</Types>", overrides.join(""), "[Content_Types].xml");
}
async function buildPptxUpdateBytes(args, exec, ctx, deck, zip) {
  const slides = args.slides ?? [];
  if (slides.length === 0) throw new Error("ppt_update needs at least one slide to append");
  validateSlideSpecs(slides);
  const presentation = readZipXmlPart(zip, "ppt/presentation.xml");
  const presentationRels = readZipXmlPart(zip, "ppt/_rels/presentation.xml.rels");
  const contentTypes = readZipXmlPart(zip, "[Content_Types].xml");
  if (presentation === null || presentationRels === null || contentTypes === null) {
    throw new Error("the .pptx is missing presentation metadata required for ppt_update");
  }
  const existingSlideFiles = zip.entryNames().filter((name2) => /^ppt\/slides\/slide[0-9]+\.xml$/.test(name2));
  if (existingSlideFiles.length === 0) throw new Error("the .pptx contains no slides");
  const layoutTarget = relationshipTarget(
    readZipXmlPart(zip, `ppt/slides/_rels/slide${slideNumber(existingSlideFiles[0])}.xml.rels`) ?? "",
    "slideLayout",
    "../slideLayouts/slideLayout1.xml"
  );
  const needsNotes = slides.some((slide) => slide.notes !== void 0 && slide.notes.trim() !== "");
  const notesMasterName = zip.entryNames().find((name2) => /^ppt\/notesMasters\/notesMaster[0-9]+\.xml$/.test(name2));
  if (needsNotes && notesMasterName === void 0) {
    throw new Error("ppt_update can add speaker notes only when the existing deck has a notes master");
  }
  const notesMasterTarget = notesMasterName === void 0 ? void 0 : `../${notesMasterName.slice("ppt/".length)}`;
  const parts = binaryPartsOf(zip);
  const existingSlideCount = existingSlideFiles.length;
  const firstSlideNumber = maxSlideFileNumber(zip) + 1;
  let nextMediaNumber = maxMediaFileNumber(zip) + 1;
  const builds = slides.map((spec) => ({ spec, first: false, images: [] }));
  for (const [slideOffset, build] of builds.entries()) {
    for (const [imageIndex, image] of (build.spec.images ?? []).entries()) {
      const mediaName = `ppt/media/image${nextMediaNumber++}.svg`;
      build.images.push(await placeImage(exec, ctx, deck, image, existingSlideCount + slideOffset, imageIndex, mediaName));
    }
  }
  const usedRelationshipIds = /* @__PURE__ */ new Set();
  for (const match of presentationRels.matchAll(/\bId="rId(\d+)"/g)) {
    usedRelationshipIds.add(Number.parseInt(match[1], 10));
  }
  let updatedPresentationRels = presentationRels;
  let updatedPresentation = presentation;
  let updatedContentTypes = contentTypes;
  const appendedSlideNumbers = [];
  const appendedNotesNumbers = [];
  const layouts = [];
  const relationshipXmlParts = [];
  const slideRefs = [];
  for (const [slideOffset, build] of builds.entries()) {
    const number = firstSlideNumber + slideOffset;
    const slideRelId = nextRelationshipId(updatedPresentationRels, usedRelationshipIds);
    const { xml, elements } = slideParts(build);
    const hasNotes = build.spec.notes !== void 0 && build.spec.notes.trim() !== "";
    const imageTargets = build.images.map((image) => `../media/${image.mediaName.split("/").at(-1)}`);
    updatedPresentationRels = appendBeforeClose(
      updatedPresentationRels,
      "</Relationships>",
      relationshipXml(slideRelId, "officeDocument/2006/relationships/slide", `slides/slide${number}.xml`, false),
      "ppt/_rels/presentation.xml.rels"
    );
    relationshipXmlParts.push({
      name: `ppt/slides/_rels/slide${number}.xml.rels`,
      content: slideRelsXml(number, imageTargets, hasNotes, layoutTarget)
    });
    slideRefs.push({ id: slideRelId });
    appendedSlideNumbers.push(number);
    if (hasNotes) appendedNotesNumbers.push(number);
    layouts.push({ index: existingSlideCount + slideOffset + 1, elements });
  }
  updatedPresentation = appendBeforeClose(
    updatedPresentation,
    "</p:sldIdLst>",
    slideRefs.map((ref, index) => `<p:sldId id="${maxNumberInXml(presentation, /<p:sldId\b[^>]*\bid="(\d+)"/g) + index + 1}" r:id="${ref.id}"/>`).join(""),
    "ppt/presentation.xml"
  );
  updatedContentTypes = appendPptContentTypes(updatedContentTypes, appendedSlideNumbers, appendedNotesNumbers, nextMediaNumber > maxMediaFileNumber(zip) + 1);
  const replacements = /* @__PURE__ */ new Map([
    ["[Content_Types].xml", updatedContentTypes],
    ["ppt/presentation.xml", updatedPresentation],
    ["ppt/_rels/presentation.xml.rels", updatedPresentationRels]
  ]);
  const outputParts = parts.map((part) => {
    const replacement = replacements.get(part.name);
    return replacement === void 0 ? part : { name: part.name, data: Buffer.from(replacement, "utf-8") };
  });
  for (const [slideOffset, build] of builds.entries()) {
    const number = firstSlideNumber + slideOffset;
    const { xml } = slideParts(build);
    outputParts.push({ name: `ppt/slides/slide${number}.xml`, data: Buffer.from(slideXml(xml), "utf-8") });
    outputParts.push({ name: relationshipXmlParts[slideOffset].name, data: Buffer.from(relationshipXmlParts[slideOffset].content, "utf-8") });
    if (build.spec.notes !== void 0 && build.spec.notes.trim() !== "") {
      outputParts.push({ name: `ppt/notesSlides/notesSlide${number}.xml`, data: Buffer.from(notesSlideXml(build.spec.notes), "utf-8") });
      outputParts.push({ name: `ppt/notesSlides/_rels/notesSlide${number}.xml.rels`, data: Buffer.from(notesRelsXml(number, notesMasterTarget), "utf-8") });
    }
    for (const image of build.images) {
      outputParts.push({ name: image.mediaName, data: Buffer.from(image.media, "utf-8") });
    }
  }
  return {
    bytes: buildBinaryZip(outputParts),
    layout: layouts,
    slideCount: existingSlideCount + builds.length,
    embeddedImages: builds.reduce((count, build) => count + build.images.length, 0)
  };
}
...
var ELEMENT_SCHEMA = {
  ...
  properties: {
    ...
    path: { type: "string", description: "Embedded media part path, when the writer placed it." },
...
var PPT_CREATE_OUTPUT = {
  ...
  properties: {
    ...
    slides: {
      ...
      description: "Per-slide element layout echo: where every text box, bullet list, and embedded image landed, in inches."
    }
  }
};
var PPT_READ_OUTPUT = {
...
var PPT_UPDATE_OUTPUT = {
  type: "object",
  additionalProperties: false,
  properties: {
    ...FILE_RESULT_SCHEMA.properties,
    slideCount: { type: "integer", required: true },
    appendedSlides: { type: "integer", required: true },
    embeddedImages: { type: "integer", required: true },
    slides: {
      type: "array",
      required: true,
      items: SLIDE_LAYOUT_SCHEMA,
      description: "Layout echo for the slides appended by this update."
    }
  }
};
function registerPptCreate(ctx) {
  return ctx.tools.register(defineTool2({
    name: "ppt_create",
    description: "Create a PowerPoint .pptx presentation in the session workspace (16:9 widescreen, 13.33 x 7.5 in). Optionally start with a title slide, then add slides with a title, body paragraphs, bullet points, speaker notes, and embedded PNG/JPG/GIF images. Images are packaged inside ppt/media as embedded SVG wrappers containing the original image bytes, so the deck has no external image dependency. Give x/y/w/h in inches for explicit placement (sizing: contain fits inside the box, cover fills it and crops), or omit them for automatic placement below the text at natural size. The result echoes every element's landing position (inches) and a text wireframe sketch of each slide, so you can verify the composition you authored.",
    parameters: {
      path: {
        type: "string",
        required: true,
        ...
        items: {
          ...
          properties: {
            ...
            images: {
              ...
              items: {
                ...
                properties: {
                  path: {
                    type: "string",
                    required: true,
                    description: "Image file inside the session workspace: .png, .jpg, .jpeg, or .gif. Its bytes are embedded in the deck; the source file is only read during creation."
                  },
              ...
              description: "Images embedded on this slide, drawn after the text content."
            }
          }
        },
...
function registerPptUpdate(ctx) {
  return ctx.tools.register(defineTool2({
    name: "ppt_update",
    description: "Append one or more slides to an existing PowerPoint .pptx in the session workspace. The existing slides and metadata remain in place; the new slides support titles, paragraphs, bullets, speaker notes, and embedded PNG/JPG/GIF images. New images are stored inside ppt/media as embedded SVG wrappers containing the original bytes and are referenced with internal r:embed relationships. The file is rewritten in place through an atomic staging-file rename, and existing binary media are preserved byte-for-byte.",
    parameters: {
      path: {
        type: "string",
        required: true,
        description: "Path to the existing .pptx file, relative to the session workspace or absolute inside it."
      },
      slides: {
        type: "array",
        required: true,
              ...
              items: {
                ...
                properties: {
                  path: {
                    type: "string",
                    required: true,
                    description: "Image file inside the session workspace: .png, .jpg, .jpeg, or .gif. Its bytes are embedded in the updated deck."
                  },
                  x: { type: "number", description: "Left position in inches on the 13.33x7.5 slide. Omit for automatic placement below the text." },
                  y: { type: "number", description: "Top position in inches. Omit for automatic placement." },
                  w: { type: "number", description: "Display width in inches. Omit to use the intrinsic size." },
                  h: { type: "number", description: "Display height in inches. Omit to use the intrinsic size; a single dimension scales by aspect." },
                  sizing: {
                    type: "string",
                    enum: ["contain", "cover"],
                    description: "Fit mode inside the w x h box: contain fits whole (default), cover fills it and crops. Requires w and h."
                  },
                  alt: { type: "string", description: "Alt text; defaults to the image path." }
                }
              },
        ...
        description: "Slides appended in the supplied order."
      }
    },
    output: {
      schema: PPT_UPDATE_OUTPUT,
      render: (_args, value) => [({
        type: "text",
        text: `Updated PowerPoint ${value.path} (${value.sizeBytes} bytes; ${value.slideCount} slide(s) total, ${value.appendedSlides} appended, ${value.embeddedImages} embedded image(s)).
` + value.slides.map((slide) => `Appended slide ${slide.index} layout:
${sketchSlide(SLIDE_WIDTH_INCHES, SLIDE_HEIGHT_INCHES, slide.elements)}`).join("\n\n")
      })]
    },
    presentCall: (args) => ({
      card: "generic",
    ...
    async execute(args, exec) {
      ...
      const zip = readZip(bytes);
      exec.signal.throwIfAborted();
      const built = await buildPptxUpdateBytes(args, exec, ctx, target, zip);
      exec.signal.throwIfAborted();
      const sizeBytes = await saveOfficeBinary(exec, ctx, target.target, built.bytes);
      return {
        path: target.display,
        sizeBytes,
        slideCount: built.slideCount,
        appendedSlides: args.slides.length,
        embeddedImages: built.embeddedImages,
        slides: built.layout
      };
    }
  }));
...
function registerPptTools(ctx) {
  const disposers = [registerPptCreate(ctx), registerPptRead(ctx), registerPptUpdate(ctx)];
  return () => disposers.forEach((dispose) => dispose());
}
...
function registerWordUpdate(ctx) {
  return ctx.tools.register(defineTool3({
    ...
    async execute(args, exec) {
      ...
      const parts = binaryPartsOf(zip).map(
        (part) => part.name === "word/document.xml" ? { name: part.name, data: Buffer.from(updated, "utf-8") } : part
      );
      exec.signal.throwIfAborted();
      const sizeBytes = await saveOfficeBinary(exec, ctx, target.target, buildBinaryZip(parts));
      return {
        path: target.display,
        sizeBytes,
...
var APP_PROPS_XML2 = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?><Properties xmlns="http://schemas.openxmlformats.org/officeDocument/2006/extended-properties"><Application>dsh-dai-office-tools</Application></Properties>';
