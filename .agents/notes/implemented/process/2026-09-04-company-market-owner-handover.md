...
1. **Ingest and review (收编).** Security-review the source, strip bypass
   ...
   `plugin-sources/dsh-free-search-0.4.184/README-hardened.zh.md` for the
...
owner applies the snippet with `accept-handoff` (the command re-checks the
PASS receipt against the local verification record — never paste by hand)
(§Publishing below).
